<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="zh_CN">
<context>
    <name></name>
    <message id="licence-not-found">
        <location filename="../Protocol/commandline.cpp" line="144"/>
        <location filename="../ClientGUI/ui/mainwindow.cpp" line="133"/>
        <location filename="../ClientGUI/ui/port/licensearea.cpp" line="33"/>
        <source>Can&apos;t find license file, exiting.</source>
        <translation>找不到许可证文件，退出。</translation>
    </message>
    <message id="naganami">
        <location filename="../Protocol/commandline.cpp" line="156"/>
        <source>What? Admiral Tanaka? He&apos;s the real deal, isn&apos;t he?
Great at battle and bad at politics--so cool!</source>
        <translation>什么？你说田中提督？他可是个人物呢，不是吗？会打仗，不会做人——挺好的嘛！</translation>
    </message>
    <message id="help-msg">
        <location filename="../Protocol/commandline.cpp" line="236"/>
        <location filename="../ClientGUI/clientv2.cpp" line="363"/>
        <source>Use &apos;exit&apos; to quit, &apos;help&apos; to show help, &apos;commands&apos; to show available commands.</source>
        <translation>使用exit退出，help查看帮助，commands查看可用命令。</translation>
    </message>
    <message id="invalid-command">
        <location filename="../Protocol/commandline.cpp" line="250"/>
        <location filename="../ClientGUI/clientv2.cpp" line="1614"/>
        <source>Invalid Command, use &apos;commands&apos; for valid commands, &apos;help&apos; for help, &apos;exit&apos; to exit.</source>
        <translation>不合理的命令，使用commands查看可用命令，help查看帮助，exit退出。</translation>
    </message>
    <message id="exit-helper">
        <location filename="../Protocol/commandline.cpp" line="255"/>
        <location filename="../ClientGUI/clientv2.cpp" line="1660"/>
        <source>Use &apos;exit&apos; to quit.</source>
        <translation>使用exit退出。</translation>
    </message>
    <message id="good-command">
        <location filename="../Protocol/commandline.cpp" line="258"/>
        <location filename="../ClientGUI/clientv2.cpp" line="1663"/>
        <source>Available commands:</source>
        <translation>可用命令：</translation>
    </message>
    <message id="all-command">
        <location filename="../Protocol/commandline.cpp" line="263"/>
        <location filename="../ClientGUI/clientv2.cpp" line="1668"/>
        <source>All commands:</source>
        <translation>全部命令：</translation>
    </message>
    <message id="goodbye">
        <location filename="../Protocol/commandline.cpp" line="393"/>
        <source>Goodbye, press ENTER to quit.</source>
        <translation>再见，使用ENTER退出。</translation>
    </message>
    <message id="equip-local-name-lack">
        <location filename="../Protocol/equipment.cpp" line="31"/>
        <source>Local language (%1) for equipment name not found!</source>
        <translation>语言（%1）的装备名未找到！</translation>
    </message>
    <message id="equip-type-lack">
        <location filename="../Protocol/equipment.cpp" line="47"/>
        <source>Fetch equipment type failure!</source>
        <translation>获取装备类型失败！</translation>
    </message>
    <message id="equip-attr-lack">
        <location filename="../Protocol/equipment.cpp" line="61"/>
        <source>Fetch equipment attributes failure!</source>
        <translation>获取装备特征失败！</translation>
    </message>
    <message id="lua-error">
        <location filename="../Protocol/equipment.cpp" line="1154"/>
        <source>The function can_equip from the file %1 has failed to run: %2</source>
        <oldsource>The code from the file %1 has failed to run: %2</oldsource>
        <translation type="unfinished"></translation>
    </message>
    <message id="SMALLGUNFLAT">
        <location filename="../Protocol/equiptype.cpp" line="35"/>
        <location filename="../Protocol/equiptype.cpp" line="166"/>
        <source>小口径主炮（平射）</source>
        <translation>小口径主炮（平射）</translation>
    </message>
    <message id="SMALLGUNFLAK">
        <location filename="../Protocol/equiptype.cpp" line="37"/>
        <location filename="../Protocol/equiptype.cpp" line="167"/>
        <source>小口径主炮（高角）</source>
        <translation>小口径主炮（高角）</translation>
    </message>
    <message id="MIDGUN">
        <location filename="../Protocol/equiptype.cpp" line="39"/>
        <location filename="../Protocol/equiptype.cpp" line="123"/>
        <location filename="../Protocol/equiptype.cpp" line="124"/>
        <location filename="../Protocol/equiptype.cpp" line="125"/>
        <source>中口径主炮</source>
        <translation>中口径主炮</translation>
    </message>
    <message id="BIGGUN">
        <location filename="../Protocol/equiptype.cpp" line="41"/>
        <location filename="../Protocol/equiptype.cpp" line="97"/>
        <source>大口径主炮</source>
        <translation>大口径主炮</translation>
    </message>
    <message id="SUPERBIGGUN">
        <location filename="../Protocol/equiptype.cpp" line="43"/>
        <location filename="../Protocol/equiptype.cpp" line="168"/>
        <location filename="../Protocol/equiptype.cpp" line="169"/>
        <source>超大口径主炮</source>
        <translation>超大口径主炮</translation>
    </message>
    <message id="SECGUN">
        <location filename="../Protocol/equiptype.cpp" line="45"/>
        <location filename="../Protocol/equiptype.cpp" line="163"/>
        <location filename="../Protocol/equiptype.cpp" line="164"/>
        <location filename="../Protocol/equiptype.cpp" line="165"/>
        <source>副炮</source>
        <translation>副炮</translation>
    </message>
    <message id="FIGHTER">
        <location filename="../Protocol/equiptype.cpp" line="47"/>
        <location filename="../Protocol/equiptype.cpp" line="114"/>
        <location filename="../Protocol/equiptype.cpp" line="115"/>
        <source>舰载战斗机</source>
        <translation>舰载战斗机</translation>
    </message>
    <message id="BOMBDIVE">
        <location filename="../Protocol/equiptype.cpp" line="49"/>
        <location filename="../Protocol/equiptype.cpp" line="98"/>
        <location filename="../Protocol/equiptype.cpp" line="99"/>
        <location filename="../Protocol/equiptype.cpp" line="100"/>
        <location filename="../Protocol/equiptype.cpp" line="101"/>
        <location filename="../Protocol/equiptype.cpp" line="102"/>
        <location filename="../Protocol/equiptype.cpp" line="103"/>
        <location filename="../Protocol/equiptype.cpp" line="104"/>
        <location filename="../Protocol/equiptype.cpp" line="105"/>
        <source>舰载爆击机</source>
        <translation>舰载爆击机</translation>
    </message>
    <message id="BOMBTORP">
        <location filename="../Protocol/equiptype.cpp" line="51"/>
        <location filename="../Protocol/equiptype.cpp" line="106"/>
        <location filename="../Protocol/equiptype.cpp" line="107"/>
        <location filename="../Protocol/equiptype.cpp" line="108"/>
        <location filename="../Protocol/equiptype.cpp" line="109"/>
        <location filename="../Protocol/equiptype.cpp" line="110"/>
        <source>舰载雷击机</source>
        <translation>舰载雷击机</translation>
    </message>
    <message id="RECON">
        <location filename="../Protocol/equiptype.cpp" line="53"/>
        <location filename="../Protocol/equiptype.cpp" line="151"/>
        <location filename="../Protocol/equiptype.cpp" line="152"/>
        <location filename="../Protocol/equiptype.cpp" line="153"/>
        <location filename="../Protocol/equiptype.cpp" line="154"/>
        <location filename="../Protocol/equiptype.cpp" line="155"/>
        <location filename="../Protocol/equiptype.cpp" line="156"/>
        <location filename="../Protocol/equiptype.cpp" line="157"/>
        <location filename="../Protocol/equiptype.cpp" line="158"/>
        <source>侦察机</source>
        <translation>侦察机</translation>
    </message>
    <message id="SEAPLANEBF">
        <location filename="../Protocol/equiptype.cpp" line="55"/>
        <location filename="../Protocol/equiptype.cpp" line="159"/>
        <location filename="../Protocol/equiptype.cpp" line="160"/>
        <location filename="../Protocol/equiptype.cpp" line="161"/>
        <location filename="../Protocol/equiptype.cpp" line="162"/>
        <source>水上爆击机/战斗机</source>
        <translation>水上爆击机/战斗机</translation>
    </message>
    <message id="TORP">
        <location filename="../Protocol/equiptype.cpp" line="57"/>
        <location filename="../Protocol/equiptype.cpp" line="170"/>
        <location filename="../Protocol/equiptype.cpp" line="171"/>
        <location filename="../Protocol/equiptype.cpp" line="172"/>
        <source>鱼雷</source>
        <translation>鱼雷</translation>
    </message>
    <message id="AA">
        <location filename="../Protocol/equiptype.cpp" line="59"/>
        <location filename="../Protocol/equiptype.cpp" line="82"/>
        <location filename="../Protocol/equiptype.cpp" line="83"/>
        <location filename="../Protocol/equiptype.cpp" line="84"/>
        <source>防空装备</source>
        <translation>防空装备</translation>
    </message>
    <message id="ASW">
        <location filename="../Protocol/equiptype.cpp" line="61"/>
        <location filename="../Protocol/equiptype.cpp" line="85"/>
        <location filename="../Protocol/equiptype.cpp" line="86"/>
        <location filename="../Protocol/equiptype.cpp" line="87"/>
        <location filename="../Protocol/equiptype.cpp" line="88"/>
        <location filename="../Protocol/equiptype.cpp" line="89"/>
        <location filename="../Protocol/equiptype.cpp" line="90"/>
        <location filename="../Protocol/equiptype.cpp" line="91"/>
        <location filename="../Protocol/equiptype.cpp" line="92"/>
        <location filename="../Protocol/equiptype.cpp" line="93"/>
        <source>对潜装备</source>
        <translation>对潜装备</translation>
    </message>
    <message id="RADAR">
        <location filename="../Protocol/equiptype.cpp" line="63"/>
        <location filename="../Protocol/equiptype.cpp" line="143"/>
        <location filename="../Protocol/equiptype.cpp" line="144"/>
        <location filename="../Protocol/equiptype.cpp" line="145"/>
        <location filename="../Protocol/equiptype.cpp" line="146"/>
        <location filename="../Protocol/equiptype.cpp" line="147"/>
        <location filename="../Protocol/equiptype.cpp" line="148"/>
        <location filename="../Protocol/equiptype.cpp" line="149"/>
        <location filename="../Protocol/equiptype.cpp" line="150"/>
        <source>雷达</source>
        <translation>雷达</translation>
    </message>
    <message id="LAND">
        <location filename="../Protocol/equiptype.cpp" line="65"/>
        <location filename="../Protocol/equiptype.cpp" line="118"/>
        <location filename="../Protocol/equiptype.cpp" line="119"/>
        <location filename="../Protocol/equiptype.cpp" line="120"/>
        <location filename="../Protocol/equiptype.cpp" line="121"/>
        <location filename="../Protocol/equiptype.cpp" line="122"/>
        <source>对陆装备</source>
        <translation>对陆装备</translation>
    </message>
    <message id="ATTACKLB">
        <location filename="../Protocol/equiptype.cpp" line="67"/>
        <location filename="../Protocol/equiptype.cpp" line="94"/>
        <location filename="../Protocol/equiptype.cpp" line="95"/>
        <location filename="../Protocol/equiptype.cpp" line="96"/>
        <source>陆基攻击机</source>
        <translation>陆基攻击机</translation>
    </message>
    <message id="FIGHTERLB">
        <location filename="../Protocol/equiptype.cpp" line="69"/>
        <location filename="../Protocol/equiptype.cpp" line="116"/>
        <location filename="../Protocol/equiptype.cpp" line="117"/>
        <source>陆基战斗机</source>
        <translation>陆基战斗机</translation>
    </message>
    <message id="BULGE">
        <location filename="../Protocol/equiptype.cpp" line="71"/>
        <location filename="../Protocol/equiptype.cpp" line="111"/>
        <location filename="../Protocol/equiptype.cpp" line="112"/>
        <location filename="../Protocol/equiptype.cpp" line="113"/>
        <source>增设装甲</source>
        <translation>增设装甲</translation>
    </message>
    <message id="PERS">
        <location filename="../Protocol/equiptype.cpp" line="73"/>
        <location filename="../Protocol/equiptype.cpp" line="141"/>
        <location filename="../Protocol/equiptype.cpp" line="142"/>
        <source>人员</source>
        <translation>人员</translation>
    </message>
    <message id="OTHER">
        <location filename="../Protocol/equiptype.cpp" line="75"/>
        <location filename="../Protocol/equiptype.cpp" line="126"/>
        <location filename="../Protocol/equiptype.cpp" line="127"/>
        <location filename="../Protocol/equiptype.cpp" line="128"/>
        <location filename="../Protocol/equiptype.cpp" line="129"/>
        <location filename="../Protocol/equiptype.cpp" line="130"/>
        <location filename="../Protocol/equiptype.cpp" line="131"/>
        <location filename="../Protocol/equiptype.cpp" line="132"/>
        <location filename="../Protocol/equiptype.cpp" line="133"/>
        <location filename="../Protocol/equiptype.cpp" line="134"/>
        <location filename="../Protocol/equiptype.cpp" line="135"/>
        <location filename="../Protocol/equiptype.cpp" line="136"/>
        <location filename="../Protocol/equiptype.cpp" line="137"/>
        <location filename="../Protocol/equiptype.cpp" line="138"/>
        <location filename="../Protocol/equiptype.cpp" line="139"/>
        <location filename="../Protocol/equiptype.cpp" line="140"/>
        <source>其他</source>
        <translation>其他</translation>
    </message>
    <message id="unknown">
        <location filename="../Protocol/equiptype.cpp" line="201"/>
        <location filename="../ClientGUI/ui/fleet/shipequip.cpp" line="158"/>
        <source>Unknown</source>
        <translation>未知</translation>
    </message>
    <message id="terminial-required">
        <location filename="../Protocol/kp.cpp" line="39"/>
        <source>This program must be run in the terminal.</source>
        <translation>此程序必须在终端内运行。</translation>
    </message>
    <message id="receive-msg-timeout">
        <location filename="../Protocol/receiver.cpp" line="82"/>
        <source>Message %1 timeouted when receiving!</source>
        <translation>接收消息%1时超时！</translation>
    </message>
    <message id="same-msg-uid-have-inconsistent-total-parts">
        <location filename="../Protocol/receiver.cpp" line="94"/>
        <source>Message total parts is inconsistent!</source>
        <translation>消息总分段数不统一！</translation>
    </message>
    <message id="msg-convert-to-json-failed">
        <location filename="../Protocol/receiver.cpp" line="123"/>
        <source>Message convert to JSON failed!</source>
        <translation>消息转换为JSON失败！</translation>
    </message>
    <message id="peerinfo-handler">
        <location filename="../Protocol/receiver.cpp" line="126"/>
        <source>PeerInfo: %1</source>
        <translation>对方主机信息：%1</translation>
    </message>
    <message id="%1 Oil">
        <source>%1 Oil </source>
        <oldsource>%1 Oil</oldsource>
        <translation type="vanished">%1油</translation>
    </message>
    <message id="%1 Ammo">
        <source>%1 Ammo </source>
        <translation type="vanished">%1弹</translation>
    </message>
    <message id="%1 Steel">
        <source>%1 Steel </source>
        <translation type="vanished">%1钢</translation>
    </message>
    <message id="%1 Rubber">
        <source>%1 Rubber </source>
        <translation type="vanished">%1橡胶</translation>
    </message>
    <message id="%1 Al">
        <source>%1 Aluminum </source>
        <translation type="vanished">%1铝</translation>
    </message>
    <message id="%1 W">
        <source>%1 Tungsten </source>
        <translation type="vanished">%1钨</translation>
    </message>
    <message id="%1 Cr">
        <source>%1 Chromium </source>
        <translation type="vanished">%1铬</translation>
    </message>
    <message id="input-buffer-empty">
        <location filename="../Protocol/sender.cpp" line="41"/>
        <source>Input buffer is empty.</source>
        <translation>输入缓冲区为空。</translation>
    </message>
    <message id="unable-send-data">
        <location filename="../Protocol/sender.cpp" line="78"/>
        <source>Unable to send data.</source>
        <translation>未能发送数据。</translation>
    </message>
    <message id="error-reading-intended-message">
        <location filename="../Protocol/sender.cpp" line="108"/>
        <source>Error reading intended message.</source>
        <translation>读取应接收消息失败。</translation>
    </message>
    <message id="unable-send-data-connection-broke">
        <location filename="../Protocol/sender.cpp" line="130"/>
        <source>Unable to send data, connection broke.</source>
        <translation>发送数据失败，连接中断。</translation>
    </message>
    <message id="write-buffer-failed-to-fill">
        <location filename="../Protocol/sender.cpp" line="136"/>
        <source>Write buffer failed to fill completely.</source>
        <translation>输出缓冲区未能充满。</translation>
    </message>
    <message id="escort">
        <location filename="../Protocol/shiptype.cpp" line="14"/>
        <source>Escort</source>
        <translation>护卫舰</translation>
    </message>
    <message id="escort-destoryer">
        <location filename="../Protocol/shiptype.cpp" line="16"/>
        <source>Escort Destroyer</source>
        <translation>护卫驱逐舰</translation>
    </message>
    <message id="light-cruiser">
        <location filename="../Protocol/shiptype.cpp" line="18"/>
        <source>Light Cruiser</source>
        <translation>轻巡洋舰</translation>
    </message>
    <message id="training-cruiser">
        <location filename="../Protocol/shiptype.cpp" line="20"/>
        <source>Training Cruiser</source>
        <translation>练习巡洋舰</translation>
    </message>
    <message id="torpedo-cruiser">
        <location filename="../Protocol/shiptype.cpp" line="22"/>
        <source>Light Cruiser (Advanced Torpedos)</source>
        <translation>重雷装巡洋舰</translation>
    </message>
    <message id="light-aviation-cruiser">
        <location filename="../Protocol/shiptype.cpp" line="24"/>
        <source>Light Cruiser (Advanced Aviation)</source>
        <translation>轻（航空）巡洋舰</translation>
    </message>
    <message id="submarine-tender">
        <location filename="../Protocol/shiptype.cpp" line="26"/>
        <source>Submarine Tender</source>
        <translation>潜水母舰</translation>
    </message>
    <message id="torpedo-cruiser-aviation">
        <location filename="../Protocol/shiptype.cpp" line="28"/>
        <source>Light Cruiser (Advanced Torpedos/Aviation)</source>
        <translation>重雷装（航空）巡洋舰</translation>
    </message>
    <message id="light-cruiser-aa">
        <location filename="../Protocol/shiptype.cpp" line="30"/>
        <source>Light Cruiser (Advanced Anti-Air)</source>
        <translation>轻（防空）巡洋舰</translation>
    </message>
    <message id="heavy-cruiser">
        <location filename="../Protocol/shiptype.cpp" line="32"/>
        <source>Heavy Cruiser</source>
        <translation>重巡洋舰</translation>
    </message>
    <message id="heavy-cruiser-torp">
        <location filename="../Protocol/shiptype.cpp" line="34"/>
        <source>Heavy Cruiser (Advanced Torpedos)</source>
        <translation>重巡洋舰（高级雷击）</translation>
    </message>
    <message id="aviation-cruiser">
        <location filename="../Protocol/shiptype.cpp" line="36"/>
        <source>Heavy Cruiser (Advanced Aviation)</source>
        <translation>航空巡洋舰</translation>
    </message>
    <message id="aviation-cruiser-torp">
        <location filename="../Protocol/shiptype.cpp" line="38"/>
        <source>Heavy Cruiser (Advanced Aviation &amp; Torpedos)</source>
        <translation>航空巡洋舰（高级雷击）</translation>
    </message>
    <message id="heavy-cruiser-aa">
        <location filename="../Protocol/shiptype.cpp" line="40"/>
        <source>Heavy Cruiser (Advanced Anti-Air)</source>
        <translation>重（防空）巡洋舰</translation>
    </message>
    <message id="battleship">
        <location filename="../Protocol/shiptype.cpp" line="42"/>
        <source>Battleship</source>
        <translation>战舰</translation>
    </message>
    <message id="battlecruiser">
        <location filename="../Protocol/shiptype.cpp" line="44"/>
        <source>Battlecruiser</source>
        <translation>巡洋战舰</translation>
    </message>
    <message id="highspeed-battleship">
        <location filename="../Protocol/shiptype.cpp" line="46"/>
        <source>Battleship (High speed)</source>
        <translation>高速战舰</translation>
    </message>
    <message id="aviation-battleship">
        <location filename="../Protocol/shiptype.cpp" line="48"/>
        <source>Battleship (Advanced Aviation)</source>
        <translation>航空战舰</translation>
    </message>
    <message id="aviation-battlecruiser">
        <location filename="../Protocol/shiptype.cpp" line="50"/>
        <source>Battlecruiser (Advanced Aviation)</source>
        <translation>航空战列巡洋舰</translation>
    </message>
    <message id="highspeed-av-battleship">
        <location filename="../Protocol/shiptype.cpp" line="52"/>
        <source>Battleship (High speed &amp; Aviation)</source>
        <translation>航空高速战舰</translation>
    </message>
    <message id="super-battleship">
        <location filename="../Protocol/shiptype.cpp" line="54"/>
        <source>Super Battleship</source>
        <translation>超级战舰</translation>
    </message>
    <message id="super-highspeed-battleship">
        <location filename="../Protocol/shiptype.cpp" line="56"/>
        <source>Super Battleship (High speed)</source>
        <translation>超级高速战舰</translation>
    </message>
    <message id="super-aviation-battleship">
        <location filename="../Protocol/shiptype.cpp" line="58"/>
        <source>Super Battleship (Advanced Aviation)</source>
        <translation>超级航空战舰</translation>
    </message>
    <message id="escort-unknown-special">
        <location filename="../Protocol/shiptype.cpp" line="63"/>
        <source>Escort (uncategorized)</source>
        <translation>护卫舰（子类未知）</translation>
    </message>
    <message id="lead-destroyer">
        <location filename="../Protocol/shiptype.cpp" line="67"/>
        <source>Lead Destroyer</source>
        <translation>向导驱逐舰</translation>
    </message>
    <message id="destroyer">
        <location filename="../Protocol/shiptype.cpp" line="71"/>
        <source>Destroyer</source>
        <translation>驱逐舰</translation>
    </message>
    <message id="light-cruiser-unknown-special">
        <location filename="../Protocol/shiptype.cpp" line="75"/>
        <source>Light Cruiser (uncategorized)</source>
        <translation>轻巡洋舰（子类未知）</translation>
    </message>
    <message id="heavy-cruiser-unknown-special">
        <location filename="../Protocol/shiptype.cpp" line="78"/>
        <source>Heavy Cruiser (uncategorized)</source>
        <translation>重巡洋舰（子类未知）</translation>
    </message>
    <message id="battleship-unknown-special">
        <location filename="../Protocol/shiptype.cpp" line="81"/>
        <source>BattleShip (uncategorized)</source>
        <translation>战舰（子类未知）</translation>
    </message>
    <message id="type-cv">
        <location filename="../Protocol/shiptype.cpp" line="85"/>
        <source>Carrier</source>
        <translation>航空母舰</translation>
    </message>
    <message id="light-carrier">
        <location filename="../Protocol/shiptype.cpp" line="89"/>
        <source>(Light)</source>
        <translation>（轻）</translation>
    </message>
    <message id="asw-carrier">
        <location filename="../Protocol/shiptype.cpp" line="92"/>
        <source>(Advanced ASW)</source>
        <oldsource>(Advanced Anti-Sub)</oldsource>
        <translation>（高级反潜）</translation>
    </message>
    <message id="escort-carrier">
        <location filename="../Protocol/shiptype.cpp" line="95"/>
        <source>(Escort)</source>
        <translation>（护卫）</translation>
    </message>
    <message id="armored-carrier">
        <location filename="../Protocol/shiptype.cpp" line="99"/>
        <source>(Armored)</source>
        <translation>（装甲）</translation>
    </message>
    <message id="night-carrier">
        <location filename="../Protocol/shiptype.cpp" line="103"/>
        <source>(Night Aviation)</source>
        <translation>（夜航）</translation>
    </message>
    <message id="type-ss">
        <location filename="../Protocol/shiptype.cpp" line="110"/>
        <source>Submarine</source>
        <translation>潜艇</translation>
    </message>
    <message id="aviation-submarine">
        <location filename="../Protocol/shiptype.cpp" line="113"/>
        <source>(Aviation)</source>
        <translation>（航空能力）</translation>
    </message>
    <message id="type-av">
        <location filename="../Protocol/shiptype.cpp" line="120"/>
        <source>Seaplane Carrier</source>
        <translation>水上机母舰</translation>
    </message>
    <message id="type-av-torp">
        <location filename="../Protocol/shiptype.cpp" line="123"/>
        <source>(Advanced Torpedos)</source>
        <translation>（高级雷击）</translation>
    </message>
    <message id="supply-ship">
        <location filename="../Protocol/shiptype.cpp" line="129"/>
        <source>Supply ship</source>
        <translation>补给舰</translation>
    </message>
    <message id="amphibious-assault">
        <location filename="../Protocol/shiptype.cpp" line="132"/>
        <source>Amphibious assault ship</source>
        <translation>两栖登陆舰</translation>
    </message>
    <message id="repair">
        <location filename="../Protocol/shiptype.cpp" line="135"/>
        <source>Repair ship</source>
        <translation>工作舰</translation>
    </message>
    <message id="type-land">
        <location filename="../Protocol/shiptype.cpp" line="138"/>
        <source>Land Structure</source>
        <translation>陆上设施</translation>
    </message>
    <message id="unknown-ship-type">
        <location filename="../Protocol/shiptype.cpp" line="141"/>
        <source>Unknown</source>
        <translation>未知舰种</translation>
    </message>
    <message id="scope-constant-less-than-1">
        <location filename="../Protocol/tech.cpp" line="69"/>
        <source>Scope constant less or equal to 1 is against design doctrine!</source>
        <translation>作用域常数小于等于1与设计意图违背！</translation>
    </message>
    <message id="tech-level-less-than-0">
        <location filename="../Protocol/tech.cpp" line="79"/>
        <source>Tech level less than 0 is against design doctrine!</source>
        <translation>科技等级小于0与设计意图违背！</translation>
    </message>
    <message id="closed-factoryslot">
        <location filename="../FactorySlot/factoryslot.cpp" line="58"/>
        <source>Closed</source>
        <translation>未开放</translation>
    </message>
    <message id="complete-factoryslot">
        <location filename="../FactorySlot/factoryslot.cpp" line="64"/>
        <source>Completed!</source>
        <translation>已完成！</translation>
    </message>
    <message id="factory-num-label">
        <location filename="../FactorySlot/factoryslot.cpp" line="87"/>
        <source>Factory %1</source>
        <translation>工厂槽%1</translation>
    </message>
    <message id="internal-server-fail">
        <location filename="../ClientGUI/clientv2.cpp" line="66"/>
        <source>Internal server initalize failed!</source>
        <translation>导入数据内部服务器初始化失败！</translation>
    </message>
    <message id="wait-for-connect-failure">
        <location filename="../ClientGUI/clientv2.cpp" line="106"/>
        <source>Failed to connect to server at %1:%2</source>
        <translation>未能连接到服务器，地址%1，端口%2</translation>
    </message>
    <message id="catbomb">
        <location filename="../ClientGUI/clientv2.cpp" line="132"/>
        <source>You have been bombarded by a cute cat.</source>
        <translation>服务器给你扔了一只可爱的猫娘。</translation>
    </message>
    <message id="connection-failed-warning">
        <location filename="../ClientGUI/clientv2.cpp" line="142"/>
        <source>Failed to establish connection, check your username, password and server status.</source>
        <translation>未能建立连接，检查你的用户名，密码和服务器的状态。</translation>
    </message>
    <message id="fscktanaka">
        <location filename="../ClientGUI/clientv2.cpp" line="187"/>
        <source>田中飞妈</source>
        <translation>田中飞妈</translation>
    </message>
    <message id="command-when-loggedout">
        <location filename="../ClientGUI/clientv2.cpp" line="259"/>
        <source>You are not online, command is invalid.</source>
        <translation>你不在线上，命令无效。</translation>
    </message>
    <message id="access-denied-login-first">
        <location filename="../ClientGUI/clientv2.cpp" line="372"/>
        <location filename="../ClientGUI/clientv2.cpp" line="386"/>
        <location filename="../ClientGUI/clientv2.cpp" line="405"/>
        <location filename="../ClientGUI/clientv2.cpp" line="418"/>
        <location filename="../ClientGUI/clientv2.cpp" line="1192"/>
        <location filename="../ClientGUI/clientv2.cpp" line="1620"/>
        <source>You have insufficient privileges (typically you need to login).</source>
        <translation>你没有所需权限（一般情况下你需要登录）。</translation>
    </message>
    <message id="change-gamestate-offline">
        <location filename="../ClientGUI/clientv2.cpp" line="546"/>
        <source>You cannot change game state while offline.</source>
        <translation>你不能在离线的时候改变游戏状态。</translation>
    </message>
    <message id="network-error">
        <location filename="../ClientGUI/clientv2.cpp" line="564"/>
        <location filename="../ClientGUI/clientv2.cpp" line="575"/>
        <location filename="../ClientGUI/networkerror.cpp" line="7"/>
        <source>Network error: %1</source>
        <translation>网络错误：%1</translation>
    </message>
    <message id="handshake-timeout">
        <location filename="../ClientGUI/clientv2.cpp" line="577"/>
        <source>%1: handshake timeout, trying to re-transmit</source>
        <translation>%1：握手超时，重新发送中</translation>
    </message>
    <message id="retransmit-toomuch">
        <location filename="../ClientGUI/clientv2.cpp" line="582"/>
        <source>%1: max restransmit time exceeded!</source>
        <translation>%1：重发次数超过限制！</translation>
    </message>
    <message id="read-dgram-failed">
        <location filename="../ClientGUI/clientv2.cpp" line="615"/>
        <location filename="../Server/server.cpp" line="561"/>
        <source>Read datagram failed due to: %1</source>
        <translation>读取数据码失败，原因：%1</translation>
    </message>
    <message id="addequip-usage">
        <location filename="../ClientGUI/clientv2.cpp" line="667"/>
        <source>Usage: addequip [equipid]</source>
        <translation>用法：addequip [装备ID]</translation>
    </message>
    <message id="develop-invalid-id">
        <location filename="../ClientGUI/clientv2.cpp" line="674"/>
        <location filename="../ClientGUI/clientv2.cpp" line="694"/>
        <source>Equipment id invalid.</source>
        <translation>装备ID不合规。</translation>
    </message>
    <message id="develop-usage">
        <location filename="../ClientGUI/clientv2.cpp" line="687"/>
        <source>Usage: develop [equipid] [FactorySlot]</source>
        <translation>用法：develop [装备ID] [装备槽位]</translation>
    </message>
    <message id="fetch-usage">
        <location filename="../ClientGUI/clientv2.cpp" line="732"/>
        <source>Usage: fetch [FactorySlot]</source>
        <translation>用法：fetch [装备槽位]</translation>
    </message>
    <message id="switch-usage">
        <location filename="../ClientGUI/clientv2.cpp" line="747"/>
        <source>Usage: switch [gamestate]</source>
        <translation>用法：switch [游戏状态]</translation>
    </message>
    <message id="game-unexpected-state">
        <location filename="../ClientGUI/clientv2.cpp" line="758"/>
        <source>Nonexistent gamestate: %1</source>
        <translation>不存在的游戏状态：%1</translation>
    </message>
    <message id="gamestate-offline">
        <location filename="../ClientGUI/clientv2.cpp" line="762"/>
        <source>Use &apos;disconnect&apos; for logout.</source>
        <translation>要登出请用disconnect。</translation>
    </message>
    <message id="goodbye-gui">
        <location filename="../ClientGUI/clientv2.cpp" line="847"/>
        <source>Goodbye.</source>
        <translation>再见。</translation>
    </message>
    <message id="connected-already">
        <location filename="../ClientGUI/clientv2.cpp" line="910"/>
        <source>Already connected, disconnect first.</source>
        <translation>已经连接，请先断开连接。</translation>
    </message>
    <message id="connect-duplicate">
        <location filename="../ClientGUI/clientv2.cpp" line="915"/>
        <source>Do not attempt duplicate connections!</source>
        <translation>不要重复尝试连接！</translation>
    </message>
    <message id="connect-usage">
        <location filename="../ClientGUI/clientv2.cpp" line="921"/>
        <source>Usage: connect [ip] [port]</source>
        <translation>用法：connect [IP] [端口]</translation>
    </message>
    <message id="ip-invalid">
        <location filename="../ClientGUI/clientv2.cpp" line="929"/>
        <location filename="../Server/server.cpp" line="3276"/>
        <source>IP isn&apos;t valid.</source>
        <translation>IP不合法。</translation>
    </message>
    <message id="port-invalid">
        <location filename="../ClientGUI/clientv2.cpp" line="936"/>
        <location filename="../Server/server.cpp" line="3282"/>
        <source>Port isn&apos;t valid, it must fall between 1024 and 49151</source>
        <translation>端口不合法，必须在1024和49151之间</translation>
    </message>
    <message id="disconnect-when-offline">
        <location filename="../ClientGUI/clientv2.cpp" line="958"/>
        <source>You are not online.</source>
        <translation>你不在线上。</translation>
    </message>
    <message id="disconnect-attempt">
        <location filename="../ClientGUI/clientv2.cpp" line="964"/>
        <source>Attempting to disconnect...</source>
        <translation>试图断连……</translation>
    </message>
    <message id="remote-disconnect">
        <location filename="../ClientGUI/clientv2.cpp" line="1074"/>
        <source>Remote disconnected.</source>
        <translation>远程主机断开连接。</translation>
    </message>
    <message id="logout-success">
        <location filename="../ClientGUI/clientv2.cpp" line="1156"/>
        <source>%1: logout success</source>
        <translation>%1：登出成功</translation>
    </message>
    <message id="logout-forced">
        <location filename="../ClientGUI/clientv2.cpp" line="1161"/>
        <source>%1: logged elsewhere, force quitting</source>
        <translation>%1：账号在别处登录，强制下线</translation>
    </message>
    <message id="logout-notonline">
        <location filename="../ClientGUI/clientv2.cpp" line="1175"/>
        <source>%1: logout failure, not online</source>
        <translation>%1：登出失败，不在线上</translation>
    </message>
    <message id="client-bad-json">
        <location filename="../ClientGUI/clientv2.cpp" line="1186"/>
        <source>Client sent a bad JSON.</source>
        <translation>客户端发送了不合理的JSON。</translation>
    </message>
    <message id="client-unsupported-json">
        <location filename="../ClientGUI/clientv2.cpp" line="1189"/>
        <source>Client sent an unsupported JSON.</source>
        <translation>客户端发送了不支持的JSON。</translation>
    </message>
    <message id="equip-not-exist">
        <location filename="../ClientGUI/clientv2.cpp" line="1197"/>
        <source>This equipment/ship does not exist.</source>
        <oldsource>This equipment does not exist.</oldsource>
        <translation type="unfinished">该装备/舰船不存在。</translation>
    </message>
    <message id="cloning-disallowed">
        <location filename="../ClientGUI/clientv2.cpp" line="1201"/>
        <source>You must use cloning vats to clone ships.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="blueprint-lack">
        <location filename="../ClientGUI/clientv2.cpp" line="1205"/>
        <source>You don&apos;t have the appropriate blueprints.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="equip-not-developable-father">
        <location filename="../ClientGUI/clientv2.cpp" line="1213"/>
        <source>This equipment requires you to possess %1 (id: %2) in order to develop.</source>
        <translation>该装备要求你拥有装备%1（ID:%2）才能开发。</translation>
    </message>
    <message id="equip-not-developable-mother">
        <location filename="../ClientGUI/clientv2.cpp" line="1223"/>
        <source>This equipment requires you to possess extra %3 skillpoints of %1 (id: %2) in order to develop.</source>
        <translation>该装备要求你拥有装备%1（ID:%2）的%3技能点才能开发。</translation>
    </message>
    <message id="factory-busy">
        <location filename="../ClientGUI/clientv2.cpp" line="1233"/>
        <source>You have not selected an available factory slot.</source>
        <translation>你没有选择一个空闲的工厂槽。</translation>
    </message>
    <message id="resource-lack">
        <location filename="../ClientGUI/clientv2.cpp" line="1237"/>
        <source>You do not have sufficient resources.</source>
        <translation>你没有足够的资源。</translation>
    </message>
    <message id="massproduction-disallowed">
        <location filename="../ClientGUI/clientv2.cpp" line="1241"/>
        <source>You have reached possessing limit for this equipment!</source>
        <translation>你达到了该装备的拥有上限！</translation>
    </message>
    <message id="production-disallowed">
        <location filename="../ClientGUI/clientv2.cpp" line="1245"/>
        <source>This equipment does not allow mass production!</source>
        <translation>该装备不能量产！</translation>
    </message>
    <message id="default-equipment-incorrect">
        <location filename="../ClientGUI/clientv2.cpp" line="1249"/>
        <source>Default equipment provided is incorrect!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-to-convert-incorrect">
        <location filename="../ClientGUI/clientv2.cpp" line="1253"/>
        <source>Ship to remodel is incorrect!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-is-disabled">
        <location filename="../ClientGUI/clientv2.cpp" line="1257"/>
        <source>Ship is disabled!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="equip-develop-failed">
        <location filename="../ClientGUI/clientv2.cpp" line="1261"/>
        <source>Equipment development failed.</source>
        <translation>装备开发失败。</translation>
    </message>
    <message id="resource-require">
        <location filename="../ClientGUI/clientv2.cpp" line="1268"/>
        <source>This operation requires %1oil/%2explosives/%3steel/%4rubber/%5aluminum/%6tungsten/%7chromium</source>
        <translation>该操作需要%1油%2弹%3钢%4橡胶%5铝%6钨%7铬</translation>
    </message>
    <message id="develop-start">
        <location filename="../ClientGUI/clientv2.cpp" line="1280"/>
        <source>Developing equipment started.</source>
        <oldsource>Start developing equipment.</oldsource>
        <translation type="unfinished">装备开发开始。</translation>
    </message>
    <message id="construct-start">
        <location filename="../ClientGUI/clientv2.cpp" line="1283"/>
        <source>Constructing ship started.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="fairy-busy">
        <location filename="../ClientGUI/clientv2.cpp" line="1287"/>
        <source>Fairy is still working on %1.</source>
        <translation>妖精还在工作，标的：%1。</translation>
    </message>
    <message id="factory-empty">
        <location filename="../ClientGUI/clientv2.cpp" line="1291"/>
        <source>Factory slot is empty.</source>
        <translation>工厂槽是空的。</translation>
    </message>
    <message id="develop-penguin">
        <location filename="../ClientGUI/clientv2.cpp" line="1296"/>
        <source>You got a cute penguin.</source>
        <translation>你得到了一只可爱的企鹅。</translation>
    </message>
    <message id="develop-success">
        <location filename="../ClientGUI/clientv2.cpp" line="1305"/>
        <source>You got new equipment %1, serial number %2</source>
        <translation>你得到了新装备%1，序列号%2</translation>
    </message>
    <message id="develop-success-id">
        <location filename="../ClientGUI/clientv2.cpp" line="1313"/>
        <source>You get new equipment with id %1, serial number %2</source>
        <translation>你得到了新装备ID%1，序列号%2</translation>
    </message>
    <message id="construct-success">
        <location filename="../ClientGUI/clientv2.cpp" line="1327"/>
        <source>You got new ship %1, serial number %2</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="construct-success-id">
        <location filename="../ClientGUI/clientv2.cpp" line="1335"/>
        <source>You get new ship with id %1, serial number %2</source>
        <oldsource>You get new equipment with id %1, serial number %2</oldsource>
        <translation type="unfinished">你得到了新装备ID%1，序列号%2</translation>
    </message>
    <message id="remodel-success">
        <location filename="../ClientGUI/clientv2.cpp" line="1350"/>
        <source>Ship serial number %2 is remodeled to %1</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="remodel-success-id">
        <location filename="../ClientGUI/clientv2.cpp" line="1357"/>
        <source>Ship serial number %2 is remodeled to id %1</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-is-disabled-normal">
        <location filename="../ClientGUI/clientv2.cpp" line="1368"/>
        <source>Ship %1 is disabled.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="server-hello">
        <location filename="../ClientGUI/clientv2.cpp" line="1374"/>
        <source>Server is alive and responding.</source>
        <translation>服务器在线且有响应。</translation>
    </message>
    <message id="client-start">
        <location filename="../ClientGUI/clientv2.cpp" line="1385"/>
        <source>You can now play the game.</source>
        <translation>你现在可以游戏了。</translation>
    </message>
    <message id="client-finish">
        <location filename="../ClientGUI/clientv2.cpp" line="1407"/>
        <source>The client can now exit normally.</source>
        <translation>客户端现在可正常退出。</translation>
    </message>
    <message id="destruct-equip-list">
        <location filename="../ClientGUI/clientv2.cpp" line="1420"/>
        <source>The following equipment are destructed: %1</source>
        <translation>以下装备被废弃：%1</translation>
    </message>
    <message id="modernize-ship-list">
        <location filename="../ClientGUI/clientv2.cpp" line="1439"/>
        <source>The following ships are modernized: %1</source>
        <oldsource>The following ship are modernized: %1</oldsource>
        <translation type="unfinished"></translation>
    </message>
    <message id="operation-success">
        <location filename="../ClientGUI/clientv2.cpp" line="1445"/>
        <source>Operation success!</source>
        <translation>操作成功！</translation>
    </message>
    <message id="fleet-size-error">
        <location filename="../ClientGUI/clientv2.cpp" line="1456"/>
        <source>Fleet is oversized or undersized.</source>
        <oldsource>Fleet is oversized.</oldsource>
        <translation>舰队规模过大或过小。</translation>
    </message>
    <message id="fleet-type-error">
        <location filename="../ClientGUI/clientv2.cpp" line="1460"/>
        <source>Fleet does not suit its type.</source>
        <translation>舰队组成与类型相悖。</translation>
    </message>
    <message id="fleet-disabled-error">
        <location filename="../ClientGUI/clientv2.cpp" line="1464"/>
        <source>Fleet contains ships unavailable for battle.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="login-success">
        <location filename="../ClientGUI/clientv2.cpp" line="1481"/>
        <source>%1: login success</source>
        <translation>%1：登录成功</translation>
    </message>
    <message id="ticket-decrypt-fail">
        <location filename="../ClientGUI/clientv2.cpp" line="1491"/>
        <source>Login failed: cannot decrypt ticket.</source>
        <translation>登录失败：无法解密票据。</translation>
    </message>
    <message id="ticket-incorrect-appid">
        <location filename="../ClientGUI/clientv2.cpp" line="1493"/>
        <source>Login failed: ticket is from incorrect app id.</source>
        <translation>登录失败：票据源APPID不对。</translation>
    </message>
    <message id="ticket-timeout">
        <location filename="../ClientGUI/clientv2.cpp" line="1495"/>
        <source>Login failed: ticket timeouted.</source>
        <translation>登录失败：票据超时。</translation>
    </message>
    <message id="steam-id-invalid">
        <location filename="../ClientGUI/clientv2.cpp" line="1497"/>
        <source>Login failed: steam id is invalid.</source>
        <translation>登录失败：Steam ID不合理。</translation>
    </message>
    <message id="steam-auth-fail">
        <location filename="../ClientGUI/clientv2.cpp" line="1499"/>
        <source>Login failed: steam authentication failed.</source>
        <translation>登录失败：Steam验证失败。</translation>
    </message>
    <message id="login-failed">
        <location filename="../ClientGUI/clientv2.cpp" line="1503"/>
        <source>%1: login failure, reason: %2</source>
        <translation>%1：登录失败，原因：%2</translation>
    </message>
    <message id="switch-cert-when-connecting">
        <location filename="../ClientGUI/clientv2.cpp" line="1676"/>
        <location filename="../Server/server.cpp" line="4354"/>
        <source>Switch certificate when connected have no effect.</source>
        <translation>在线时转换证书没有效果。</translation>
    </message>
    <message id="client-pem">
        <location filename="../ClientGUI/clientv2.cpp" line="1687"/>
        <source>Client PEM is now %1.</source>
        <translation>客户端PEM现在是%1。</translation>
    </message>
    <message id="equipment-cache-length">
        <location filename="../ClientGUI/clientv2.cpp" line="1713"/>
        <source>Equipment cache length: %1</source>
        <translation>装备缓存长度：%1</translation>
    </message>
    <message id="map-cache-length">
        <location filename="../ClientGUI/clientv2.cpp" line="1744"/>
        <source>Map cache length: %1</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="shipment-cache-length">
        <location filename="../ClientGUI/clientv2.cpp" line="1774"/>
        <source>Ship cache length: %1</source>
        <translation>舰船缓存长度：%1</translation>
    </message>
    <message id="equip-idle">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="396"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="401"/>
        <source>Idle</source>
        <translation type="unfinished">空闲</translation>
    </message>
    <message id="equip-uuid">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="447"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="563"/>
        <location filename="../ClientGUI/model/specequipmodel.cpp" line="59"/>
        <source>Equipment UUID</source>
        <translation>装备序列号</translation>
    </message>
    <message id="equip-name">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="451"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="565"/>
        <source>Equipment name</source>
        <translation>装备名</translation>
    </message>
    <message id="equip-star">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="455"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="567"/>
        <source>Improvement level</source>
        <oldsource>Equipment improvement level</oldsource>
        <translation type="unfinished">装备改修度</translation>
    </message>
    <message id="equip-attr">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="459"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="569"/>
        <source>Attributes</source>
        <oldsource>Equipment attributes</oldsource>
        <translation type="unfinished">装备属性</translation>
    </message>
    <message id="destruct">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="463"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="571"/>
        <source>Destruct</source>
        <oldsource>Destruct this equipment</oldsource>
        <translation type="unfinished">废弃该装备</translation>
    </message>
    <message id="equip-improve">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="467"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="573"/>
        <source>Improve</source>
        <oldsource>Improve this equipment</oldsource>
        <translation type="unfinished">改修该装备</translation>
    </message>
    <message id="equip-select">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="471"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="575"/>
        <source>Select</source>
        <oldsource>Select this equipment</oldsource>
        <translation type="unfinished">选择该装备</translation>
    </message>
    <message id="equip-pos">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="475"/>
        <location filename="../ClientGUI/model/equipmodel.cpp" line="577"/>
        <source>Position</source>
        <translation type="unfinished">装备位置</translation>
    </message>
    <message id="steam-60-sec">
        <location filename="../ClientGUI/steamauth.cpp" line="24"/>
        <source>Steam API &apos;RequestEncryptedAppTicket&apos; is subject to a 60 second rate limit.</source>
        <translation>Steam API “RequestEncryptedAppticket” 两次之间有至少60秒的限制。</translation>
    </message>
    <message id="steam-bIOFailure">
        <location filename="../ClientGUI/steamauth.cpp" line="40"/>
        <source>There has been an IO Failure when requesting the Encrypted App Ticket.</source>
        <oldsource>There has been an IO Failure when requesting the Encrypted App Ticket.
</oldsource>
        <translation>请求加密App票据时出现读写失败。</translation>
    </message>
    <message id="appticket-success">
        <location filename="../ClientGUI/steamauth.cpp" line="55"/>
        <source>GetEncryptedAppTicket success!</source>
        <translation>请求加密App票据成功！</translation>
    </message>
    <message id="appticket-failure">
        <location filename="../ClientGUI/steamauth.cpp" line="60"/>
        <source>GetEncryptedAppTicket failed!</source>
        <translation>请求加密App票据失败！</translation>
    </message>
    <message id="k_EResultNoConnection">
        <location filename="../ClientGUI/steamauth.cpp" line="67"/>
        <source>Calling RequestEncryptedAppTicket while not connected to steam results in this error.</source>
        <translation>请求加密App票据时没能连接到Steam。</translation>
    </message>
    <message id="display-user-name">
        <location filename="../ClientGUI/steamauth.cpp" line="69"/>
        <source>Steam ID: %1</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="k_EResultDuplicateRequest">
        <location filename="../ClientGUI/steamauth.cpp" line="74"/>
        <source>Calling RequestEncryptedAppTicket while there is already a pending request results in this error.</source>
        <translation>请求加密App票据时已经有未处理的请求。</translation>
    </message>
    <message id="k_EResultLimitExceeded">
        <location filename="../ClientGUI/steamauth.cpp" line="79"/>
        <source>Calling RequestEncryptedAppTicket more than once per minute returns this error.</source>
        <translation>请求加密App票据时在60秒内重复请求。</translation>
    </message>
    <message id="request-app-ticket-fail-unknown">
        <location filename="../ClientGUI/steamauth.cpp" line="84"/>
        <source>Calling RequestEncryptedAppTicket encountered unknown error %1.</source>
        <translation>请求加密App票据时出现未知错误%1。</translation>
    </message>
    <message id="equipview-search">
        <location filename="../ClientGUI/ui/views/equipselect.cpp" line="16"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="19"/>
        <source>Search:</source>
        <translation>搜索：</translation>
    </message>
    <message id="equipview-type">
        <location filename="../ClientGUI/ui/views/equipselect.cpp" line="22"/>
        <source>Equip type:</source>
        <translation>装备类型：</translation>
    </message>
    <message id="equipview-equip">
        <location filename="../ClientGUI/ui/views/equipselect.cpp" line="31"/>
        <source>Equip:</source>
        <translation>装备：</translation>
    </message>
    <message id="retrieving-please-wait">
        <location filename="../ClientGUI/ui/navigator.cpp" line="19"/>
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="61"/>
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="241"/>
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="284"/>
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="324"/>
        <source>Retrieving data, please wait...</source>
        <translation>获取数据中，请等待……</translation>
    </message>
    <message id="equipview-unselect">
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="71"/>
        <source>Unselect</source>
        <translation>取消选择</translation>
    </message>
    <message id="no-ship">
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="167"/>
        <source>No suitable Ship</source>
        <translation>没有符合的舰船</translation>
    </message>
    <message id="destruct-button">
        <location filename="../ClientGUI/ui/views/equipselect.cpp" line="52"/>
        <source>Destruct</source>
        <translation>废弃</translation>
    </message>
    <message id="add-star-button">
        <location filename="../ClientGUI/ui/views/equipselect.cpp" line="54"/>
        <source>Improve</source>
        <translation>改修</translation>
    </message>
    <message id="all-equipments">
        <location filename="../ClientGUI/ui/factory/developwindow.cpp" line="24"/>
        <location filename="../ClientGUI/ui/factory/developwindow.cpp" line="83"/>
        <location filename="../ClientGUI/ui/navigator.cpp" line="39"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="53"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="427"/>
        <location filename="../ClientGUI/ui/views/equipselect.cpp" line="76"/>
        <source>All equipments</source>
        <translation>所有装备</translation>
    </message>
    <message id="develop-resource-amount-unknown">
        <location filename="../ClientGUI/ui/factory/developwindow.cpp" line="131"/>
        <source>Unknown</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="no-equip">
        <location filename="../ClientGUI/ui/navigator.cpp" line="80"/>
        <location filename="../ClientGUI/ui/views/equipview.cpp" line="163"/>
        <source>No suitable Equipment</source>
        <translation>没有符合的装备</translation>
    </message>
    <message id="naganami-words">
        <location filename="../ClientGUI/ui/port/licensearea.cpp" line="47"/>
        <source>What? Admiral Tanaka? He&apos;s the real deal, isn&apos;t he?
Great at battle and bad at politics--so cool!</source>
        <translation>什么？你说田中提督？他可是个人物呢，不是吗？
会打仗，不会做人——挺好的嘛！</translation>
    </message>
    <message id="license-continue">
        <location filename="../ClientGUI/ui/port/licensearea.cpp" line="54"/>
        <source>Continue</source>
        <translation>继续游戏</translation>
    </message>
    <message id="load-equip">
        <location filename="../ClientGUI/ui/port/portarea.cpp" line="14"/>
        <source>Loading Equipment Registry...</source>
        <translation>加载装备信息……</translation>
    </message>
    <message id="load-ship">
        <location filename="../ClientGUI/ui/port/portarea.cpp" line="37"/>
        <source>Loading Ship Registry...</source>
        <translation>加载舰船信息……</translation>
    </message>
    <message id="load-map">
        <location filename="../ClientGUI/ui/port/portarea.cpp" line="42"/>
        <source>Loading Maps...</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="load-assets">
        <location filename="../ClientGUI/ui/port/portarea.cpp" line="47"/>
        <source>Loading Assets...</source>
        <translation>加载美术资源……</translation>
    </message>
    <message id="client-hello">
        <location filename="../ClientGUI/ui/port/portarea.cpp" line="52"/>
        <source>Hello!</source>
        <translation>欢迎！</translation>
    </message>
    <message id="shipview-nation">
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="25"/>
        <source>Nationality:</source>
        <translation>国籍：</translation>
    </message>
    <message id="shipview-type">
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="29"/>
        <source>Type:</source>
        <translation>舰种：</translation>
    </message>
    <message id="shipview-class">
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="33"/>
        <source>Class:</source>
        <translation>舰级：</translation>
    </message>
    <message id="add-star-button-ship">
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="53"/>
        <source>Modernize</source>
        <translation>近代化改造</translation>
    </message>
    <message id="all-nationality">
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="48"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="204"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="59"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="464"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="80"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="118"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="128"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="140"/>
        <source>All nationalities</source>
        <oldsource>All nationality</oldsource>
        <translation>所有国籍</translation>
    </message>
    <message id="wait-for-blueprint">
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="64"/>
        <source>Fetching ship blueprint data, please wait...</source>
        <oldsource>Fetching ship bluepring data, please wait...</oldsource>
        <translation type="unfinished"></translation>
    </message>
    <message id="wait-for-equip">
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="76"/>
        <source>Fetching equipment data, please wait...</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="techview-toequip">
        <location filename="../ClientGUI/ui/techview.cpp" line="177"/>
        <source>Switch to Equip</source>
        <translation>转到装备</translation>
    </message>
    <message id="Serial-num">
        <location filename="../ClientGUI/ui/techview.cpp" line="290"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="327"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="391"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="520"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="524"/>
        <source>Serial Num</source>
        <translation>序列号</translation>
    </message>
    <message id="Equip-name-def">
        <location filename="../ClientGUI/ui/techview.cpp" line="292"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="327"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="391"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="520"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="524"/>
        <source>Name</source>
        <translation>装备名</translation>
    </message>
    <message id="Equip-tech-level">
        <location filename="../ClientGUI/ui/techview.cpp" line="294"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="328"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="392"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="521"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="525"/>
        <source>Tech</source>
        <translation>科技等级</translation>
    </message>
    <message id="Weight">
        <location filename="../ClientGUI/ui/techview.cpp" line="296"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="328"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="392"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="521"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="525"/>
        <source>Weight</source>
        <translation>权重</translation>
    </message>
    <message id="db-error">
        <location filename="../Server/kerrors.cpp" line="11"/>
        <source>Database Error: %1</source>
        <translation>数据库错误：%1</translation>
    </message>
    <message id="dtls-1.2">
        <location filename="../Server/server.cpp" line="32"/>
        <source>DTLS 1.2.</source>
        <translation>DTLS 1.2。</translation>
    </message>
    <message id="dtls-1.2+">
        <location filename="../Server/server.cpp" line="36"/>
        <source>DTLS 1.2 or later.</source>
        <translation>DTLS 1.2或更新。</translation>
    </message>
    <message id="protocol-unknown">
        <location filename="../Server/server.cpp" line="40"/>
        <source>Unknown protocol.</source>
        <translation>未知协议。</translation>
    </message>
    <message id="connection-info-serverside">
        <location filename="../Server/server.cpp" line="44"/>
        <source>Session cipher: %1; session protocol: %2.</source>
        <translation>会话密钥：%1，协议：%2。</translation>
    </message>
    <message id="already-listening">
        <location filename="../Server/server.cpp" line="426"/>
        <location filename="../Server/server.cpp" line="3270"/>
        <source>Server is already listening.</source>
        <translation>服务器已在监听。</translation>
    </message>
    <message id="equip-init-failure">
        <location filename="../Server/server.cpp" line="440"/>
        <source>Equipment init failed!</source>
        <translation>初始化装备失败！</translation>
    </message>
    <message id="ship-init-failure">
        <location filename="../Server/server.cpp" line="444"/>
        <source>Ship init failed!</source>
        <translation>初始化舰船失败！</translation>
    </message>
    <message id="map-init-failure">
        <location filename="../Server/server.cpp" line="448"/>
        <source>Map init failed!</source>
        <translation>初始化地图失败！</translation>
    </message>
    <message id="importcsv-usage">
        <location filename="../Server/server.cpp" line="519"/>
        <source>Usage: importcsv [equip|ship|map]</source>
        <oldsource>Usage: importcsv [equip|ship]</oldsource>
        <translation>用法：importcsv [equip|ship|map]</translation>
    </message>
    <message id="read-peerinfo-failed">
        <location filename="../Server/server.cpp" line="568"/>
        <source>Failed to extract peer info (address, port).</source>
        <translation>未能获取远程主机信息（地址，端口）。</translation>
    </message>
    <message id="client-dc">
        <location filename="../Server/server.cpp" line="582"/>
        <source>%1: disconnected abruptly.</source>
        <translation>%1：突然断开连接。</translation>
    </message>
    <message id="user-calculate-tech-failed">
        <location filename="../Server/server.cpp" line="642"/>
        <location filename="../Server/server.cpp" line="714"/>
        <source>Calculate technology for user %1 failed!</source>
        <translation>为用户%1计算科技等级失败！</translation>
    </message>
    <message id="local-tech-bad-equipdef">
        <location filename="../Server/server.cpp" line="666"/>
        <source>Local technology computation failed due to bad equipment ID!</source>
        <translation>装备ID不存在，特定科技等级计算失败！</translation>
    </message>
    <message id="equipid-invalid-skill-points-effect">
        <location filename="../Server/server.cpp" line="801"/>
        <source>Skill points effect calculation failed due to invalid equipment ID!</source>
        <translation>装备ID不存在，技能点计算失败！</translation>
    </message>
    <message id="user-get-equip-list-failed">
        <location filename="../Server/server.cpp" line="866"/>
        <source>Get user %1&apos;s equipment list failed!</source>
        <translation>获取用户%1的装备列表失败！</translation>
    </message>
    <message id="user-get-ship-list-failed">
        <location filename="../Server/server.cpp" line="1071"/>
        <location filename="../Server/server.cpp" line="1168"/>
        <source>Get user %1&apos;s ship list failed!</source>
        <translation>获取用户%1的舰船列表失败！</translation>
    </message>
    <message id="client-id-received">
        <location filename="../Server/server.cpp" line="1238"/>
        <source>PSK callback, received a client&apos;s identity: &apos;%1&apos;</source>
        <translation>PSK回调，获得客户端ID：%1</translation>
    </message>
    <message id="wait-for-dc">
        <location filename="../Server/server.cpp" line="1265"/>
        <source>Disconnect success: %1 port %2</source>
        <translation>断开连接成功：地址%1端口%2</translation>
    </message>
    <message id="dc-failed">
        <location filename="../Server/server.cpp" line="1270"/>
        <source>Disconnect failed! %1 port %2</source>
        <translation>断开连接失败！地址%1端口%2</translation>
    </message>
    <message id="user-get-equip-list-failed-eidbased">
        <location filename="../Server/server.cpp" line="1302"/>
        <source>Get user&apos;s equipment list by uuid %1 failed!</source>
        <translation>获取用户装备，序列号%1失败！</translation>
    </message>
    <message id="improve-beyond-possible">
        <location filename="../Server/server.cpp" line="1311"/>
        <source>Equip id %1: not allowed to improve beyond possible stars.</source>
        <oldsource>Equip id %1: not allowed to improve beyond 15 stars.</oldsource>
        <translation>装备序列号%1：改修星数已到上限。</translation>
    </message>
    <message id="user-add-equip-star-failed-eidbased">
        <location filename="../Server/server.cpp" line="1323"/>
        <source>Improve equipment failed due to bad equipment uuid!</source>
        <translation>由于序列号不合理，装备改修失败！</translation>
    </message>
    <message id="improve-success">
        <location filename="../Server/server.cpp" line="1330"/>
        <source>Equip id %1: improved to %2 stars.</source>
        <translation>装备%1：改修至%2星。</translation>
    </message>
    <message id="clear-negative-skillpoints-failed">
        <location filename="../Server/server.cpp" line="1362"/>
        <source>User %1: clear negative skill points failed!</source>
        <translation>用户%1：清除负技能点失败！</translation>
    </message>
    <message id="delete-all-equip-failed">
        <location filename="../Server/server.cpp" line="1400"/>
        <source>User id %1: delete all equipment failed!</source>
        <translation>用户%1：清除所有装备失败！</translation>
    </message>
    <message id="delete-all-equip">
        <location filename="../Server/server.cpp" line="1406"/>
        <source>User id %1: all equipment deleted</source>
        <translation>用户%1：清除所有装备</translation>
    </message>
    <message id="delete-all-ship-failed">
        <location filename="../Server/server.cpp" line="1419"/>
        <source>User id %1: delete all ship failed!</source>
        <translation>用户%1：清除所有舰船失败！</translation>
    </message>
    <message id="delete-all-ship">
        <location filename="../Server/server.cpp" line="1425"/>
        <source>User id %1: all ship deleted</source>
        <translation>用户%1：清除所有舰船</translation>
    </message>
    <message id="dbfail-constructing-query-existing-bps">
        <location filename="../Server/server.cpp" line="1468"/>
        <source>Database failed when constructing: query existing ship blueprints failed!</source>
        <oldsource>Database failed when constructing: query existing ship blueprintss failed!</oldsource>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-constructing-query-existing-models">
        <location filename="../Server/server.cpp" line="1511"/>
        <source>Database failed when constructing: query existing models failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-constructing-query-existing-equips">
        <location filename="../Server/server.cpp" line="1719"/>
        <source>Database failed when constructing: query existing equips failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-constructing">
        <location filename="../Server/server.cpp" line="1575"/>
        <source>Database failed when constructing.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-constructing-query-existing-ships">
        <location filename="../Server/server.cpp" line="1598"/>
        <source>Database failed when constructing: query existing ships failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-modernizing">
        <location filename="../Server/server.cpp" line="1665"/>
        <source>Database failed when modernizing.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-modernizing-prev-lock">
        <location filename="../Server/server.cpp" line="1629"/>
        <source>Database failed when modernizing (locking previous ship).</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-constructing-delete-existing-bps">
        <location filename="../Server/server.cpp" line="1765"/>
        <source>Database failed when constructing: delete existing ship blueprints failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dbfail-developing">
        <location filename="../Server/server.cpp" line="1895"/>
        <source>Database failed when developing.</source>
        <translation>开发时数据库错误。</translation>
    </message>
    <message id="fetch-facto-failed">
        <location filename="../Server/server.cpp" line="1925"/>
        <source>Fetch factory product failed.</source>
        <translation>获取工厂结果失败。</translation>
    </message>
    <message id="dbfail-fetching">
        <location filename="../Server/server.cpp" line="2015"/>
        <source>Database failed when fetching product.</source>
        <translation>获取工厂结果时数据库错误。</translation>
    </message>
    <message id="database-uninit">
        <location filename="../Server/server.cpp" line="2026"/>
        <location filename="../Server/server.cpp" line="2077"/>
        <location filename="../Server/server.cpp" line="2202"/>
        <location filename="../Server/server.cpp" line="2339"/>
        <location filename="../Server/server.cpp" line="2442"/>
        <location filename="../Server/server.cpp" line="2505"/>
        <location filename="../Server/server.cpp" line="2653"/>
        <location filename="../Server/server.cpp" line="3955"/>
        <source>Database uninitialized!</source>
        <translation>数据库未初始化！</translation>
    </message>
    <message id="equip-refresh-failed">
        <location filename="../Server/server.cpp" line="2033"/>
        <location filename="../Server/server.cpp" line="2084"/>
        <source>Load equipment table failed!</source>
        <translation>加载装备列表失败！</translation>
    </message>
    <message id="equip-load-good">
        <location filename="../Server/server.cpp" line="2048"/>
        <source>Load equipment registry success!</source>
        <translation>加载装备列表成功！</translation>
    </message>
    <message id="equip-child-load-good">
        <location filename="../Server/server.cpp" line="2055"/>
        <source>Load equipment child list success!</source>
        <translation>加载子装备列表成功！</translation>
    </message>
    <message id="server-shutdown">
        <location filename="../Server/server.cpp" line="2062"/>
        <source>Server is shutting down</source>
        <translation>服务器关闭中</translation>
    </message>
    <message id="bad-csv">
        <location filename="../Server/server.cpp" line="2070"/>
        <location filename="../Server/server.cpp" line="2211"/>
        <location filename="../Server/server.cpp" line="2348"/>
        <location filename="../Server/server.cpp" line="2451"/>
        <location filename="../Server/server.cpp" line="2514"/>
        <source>%1: CSV file cannot be opened</source>
        <translation>%1：CSV文件无法打开</translation>
    </message>
    <message id="equip-export-good">
        <location filename="../Server/server.cpp" line="2111"/>
        <source>Export equipment registry success!</source>
        <translation>导出装备注册表成功！</translation>
    </message>
    <message id="equip-def-incomplete">
        <location filename="../Server/server.cpp" line="2231"/>
        <source>incomplete equip type definition</source>
        <translation>装备定义不全</translation>
    </message>
    <message id="equip-import-failed">
        <location filename="../Server/server.cpp" line="2248"/>
        <location filename="../Server/server.cpp" line="2268"/>
        <location filename="../Server/server.cpp" line="2286"/>
        <location filename="../Server/server.cpp" line="2303"/>
        <source>Import equipment database failed!</source>
        <translation>导入装备数据库失败！</translation>
    </message>
    <message id="num-of-equip-imports">
        <location filename="../Server/server.cpp" line="2314"/>
        <source>Imported %1 equipment(s)</source>
        <translation>导入了%1个装备</translation>
    </message>
    <message id="equip-import-good">
        <location filename="../Server/server.cpp" line="2322"/>
        <source>Import equipment registry success!</source>
        <translation>导入装备注册表成功！</translation>
    </message>
    <message id="map-node-import-failed">
        <location filename="../Server/server.cpp" line="2375"/>
        <location filename="../Server/server.cpp" line="2396"/>
        <location filename="../Server/server.cpp" line="2417"/>
        <location filename="../Server/server.cpp" line="2481"/>
        <source>Import map node database failed!</source>
        <translation>导入海域点数据库失败！</translation>
    </message>
    <message id="num-of-map-node-imports">
        <location filename="../Server/server.cpp" line="2426"/>
        <source>Imported %1 map node(s)</source>
        <translation>导入了%1个海域</translation>
    </message>
    <message id="map-node-import-good">
        <location filename="../Server/server.cpp" line="2434"/>
        <source>Import map node registry success!</source>
        <translation>导入海域点数据库成功！</translation>
    </message>
    <message id="num-of-map-relation-imports">
        <location filename="../Server/server.cpp" line="2489"/>
        <source>Imported %1 map relation(s)</source>
        <translation>导入了%1个海域关系</translation>
    </message>
    <message id="map-relation-import-good">
        <location filename="../Server/server.cpp" line="2497"/>
        <source>Import map relation registry success!</source>
        <translation>导入海域关系数据库成功！</translation>
    </message>
    <message id="ship-import-failed">
        <location filename="../Server/server.cpp" line="2549"/>
        <location filename="../Server/server.cpp" line="2566"/>
        <location filename="../Server/server.cpp" line="2586"/>
        <location filename="../Server/server.cpp" line="2610"/>
        <source>Import ship database failed!</source>
        <oldsource>Ship import failed!</oldsource>
        <translation>导入舰船数据库失败！</translation>
    </message>
    <message id="num-of-ship-imports">
        <location filename="../Server/server.cpp" line="2620"/>
        <source>Imported %1 ship(s)</source>
        <translation>导入了%1个舰船</translation>
    </message>
    <message id="ship-import-good">
        <location filename="../Server/server.cpp" line="2627"/>
        <source>Import ship registry success!</source>
        <translation>导入舰船数据库成功！</translation>
    </message>
    <message id="lua-canequip-error">
        <location filename="../Server/server.cpp" line="2639"/>
        <source>The code from the file %1 has failed to run: %2</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="lua-canequip-success">
        <location filename="../Server/server.cpp" line="2644"/>
        <source>Load equipability table success!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="map-refresh-failed">
        <location filename="../Server/server.cpp" line="2660"/>
        <location filename="../Server/server.cpp" line="2677"/>
        <location filename="../Server/server.cpp" line="2722"/>
        <location filename="../Server/server.cpp" line="2749"/>
        <source>Load map table failed!</source>
        <translation>加载地图列表失败！</translation>
    </message>
    <message id="map-load-good">
        <location filename="../Server/server.cpp" line="2776"/>
        <source>Load map registry success!</source>
        <translation>加载地图数据库成功！</translation>
    </message>
    <message id="equipid-dont-exist">
        <location filename="../Server/server.cpp" line="2798"/>
        <source>Equip id %1 don&apos;t exist!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="shipoldid-dont-exist">
        <location filename="../Server/server.cpp" line="2813"/>
        <source>Ship old id %1 don&apos;t exist!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="user-migrate-equip-failed">
        <location filename="../Server/server.cpp" line="2877"/>
        <location filename="../Server/server.cpp" line="2896"/>
        <source>User %1: import equip from KC failed, error %2</source>
        <translation>用户%1：导入舰C装备失败，错误%2</translation>
    </message>
    <message id="user-migrate-ship-failed">
        <location filename="../Server/server.cpp" line="2919"/>
        <location filename="../Server/server.cpp" line="2940"/>
        <location filename="../Server/server.cpp" line="2958"/>
        <source>User %1: import ship from KC failed, error %2</source>
        <translation>用户%1：导入舰C舰船失败，错误%2</translation>
    </message>
    <message id="import-kc-data-success">
        <location filename="../Server/server.cpp" line="2964"/>
        <source>User %1: import from KC data success!</source>
        <translation>用户%1：导入舰C数据成功！</translation>
    </message>
    <message id="modernize-ship-nonexistent">
        <location filename="../Server/server.cpp" line="2986"/>
        <source>User id %1: ship %2 does not exist when modernizing!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="modernize-ship-failed-def">
        <location filename="../Server/server.cpp" line="3005"/>
        <source>User id %1: using blueprint of ship definition %2 failed when modernizing!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="modernize-ship-def">
        <location filename="../Server/server.cpp" line="3013"/>
        <source>User id %1: using blueprint of ship definition %2 when modernizing</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="modernize-ship-failed">
        <location filename="../Server/server.cpp" line="3028"/>
        <source>User id %1: modernize ship %2 failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="modernize-ship">
        <location filename="../Server/server.cpp" line="3036"/>
        <source>User id %1: modernized ship %2 by 1 level</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="user-query-ship-fail">
        <location filename="../Server/server.cpp" line="3057"/>
        <source>Query ship %2 for user %1 failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="remodel-ship-failed">
        <location filename="../Server/server.cpp" line="3109"/>
        <source>User id %1: remodel ship failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="remodeled-ship">
        <location filename="../Server/server.cpp" line="3116"/>
        <source>User id %1: remodeled ship %2 definition %3</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="user-query-regen-time-fail">
        <location filename="../Server/server.cpp" line="3136"/>
        <source>Query last regeneration time for user %1 failed!</source>
        <translation>查询用户%1的上次自然恢复时间失败！</translation>
    </message>
    <message id="regen-min">
        <location filename="../Server/server.cpp" line="3161"/>
        <source>%1 minute(s) passed for regeneration purposes.</source>
        <translation>自然恢复已经过%1分钟。</translation>
    </message>
    <message id="natural-regen-failed">
        <location filename="../Server/server.cpp" line="3182"/>
        <source>User ID %1: natural regeneration failed!</source>
        <translation>用户ID%1：自然回复失败！</translation>
    </message>
    <message id="natural-regen">
        <location filename="../Server/server.cpp" line="3188"/>
        <source>User ID %1: natural regeneration</source>
        <translation>用户ID%1：自然回复</translation>
    </message>
    <message id="listen-usage">
        <location filename="../Server/server.cpp" line="3266"/>
        <source>Usage: listen [ip] [port]</source>
        <translation>用法：listen [ip] [端口]</translation>
    </message>
    <message id="no-cert">
        <location filename="../Server/server.cpp" line="3293"/>
        <source>Server lack a certificate.</source>
        <translation>服务器没有安全证书。</translation>
    </message>
    <message id="no-private-key">
        <location filename="../Server/server.cpp" line="3303"/>
        <source>Server lack a private key.</source>
        <translation>服务器没有私钥。</translation>
    </message>
    <message id="corrupt-private-key">
        <location filename="../Server/server.cpp" line="3311"/>
        <source>Server private key can&apos;t be read.</source>
        <translation>服务器读取私钥失败。</translation>
    </message>
    <message id="server-listen">
        <location filename="../Server/server.cpp" line="3326"/>
        <source>Server is listening on address %1 and port %2</source>
        <translation>服务器在地址%1和端口%2上监听</translation>
    </message>
    <message id="server-listen-fail">
        <location filename="../Server/server.cpp" line="3340"/>
        <source>Server failed to listen on address %1 and port %2</source>
        <translation>服务器在地址%1和端口%2上监听失败</translation>
    </message>
    <message id="server-stop">
        <location filename="../Server/server.cpp" line="3350"/>
        <source>Server stopped listening.</source>
        <translation>服务器停止监听。</translation>
    </message>
    <message id="server-stopped-already">
        <location filename="../Server/server.cpp" line="3354"/>
        <source>Server isn&apos;t listening.</source>
        <translation>服务器未在监听。</translation>
    </message>
    <message id="no-app-secret-key">
        <location filename="../Server/server.cpp" line="3376"/>
        <source>Server lack the steam app secret key.</source>
        <translation>服务器没有steam的app私钥。</translation>
    </message>
    <message id="ticket-decrypt-failed">
        <location filename="../Server/server.cpp" line="3406"/>
        <source>%1: Ticket failed to decrypt</source>
        <translation>%1：解密凭据失败</translation>
    </message>
    <message id="ticket-appid-wrong">
        <location filename="../Server/server.cpp" line="3420"/>
        <source>%1: Ticket is not from correct App ID</source>
        <translation>%1：凭据不是来自正确的App ID</translation>
    </message>
    <message id="ticket-appid-right">
        <location filename="../Server/server.cpp" line="3429"/>
        <source>Ticket decrypt from correct App ID</source>
        <translation>凭据来自正确的App ID</translation>
    </message>
    <message id="time-gone">
        <location filename="../Server/server.cpp" line="3438"/>
        <source>Elapsed: %1 second(s)</source>
        <translation>经过时间：%1秒</translation>
    </message>
    <message id="request-timeout">
        <location filename="../Server/server.cpp" line="3441"/>
        <source>%1: Request timeout</source>
        <translation>%1：请求超时</translation>
    </message>
    <message id="steam-id-wrong">
        <location filename="../Server/server.cpp" line="3455"/>
        <source>%1: Steam ID invalid</source>
        <translation>%1：SteamID不正确</translation>
    </message>
    <message id="user-login">
        <location filename="../Server/server.cpp" line="3466"/>
        <source>User login: %1</source>
        <oldsource>User login: %1 (%2)</oldsource>
        <translation type="unfinished">用户登录：%1</translation>
    </message>
    <message id="superuser-login">
        <location filename="../Server/server.cpp" line="3473"/>
        <source>Superuser login: %1</source>
        <translation>管理员用户登录：%1</translation>
    </message>
    <message id="add-user-fail">
        <location filename="../Server/server.cpp" line="3553"/>
        <source>%1: Add user failure!</source>
        <translation>%1：添加用户失败！</translation>
    </message>
    <message id="Connection-not-properly-online">
        <location filename="../Server/server.cpp" line="3594"/>
        <source>User is not properly online!</source>
        <translation>用户没在线上！</translation>
    </message>
    <message id="invalid-uid">
        <location filename="../Server/server.cpp" line="3600"/>
        <source>Invalid-uid: %1</source>
        <translation>不正确的Steam uid:%1</translation>
    </message>
    <message id="gamestate-invalid">
        <location filename="../Server/server.cpp" line="3636"/>
        <source>Game state %1 not supported!</source>
        <translation>不支持游戏状态%1！</translation>
    </message>
    <message id="command-type-wrong">
        <location filename="../Server/server.cpp" line="3718"/>
        <source>User %1: command type not supported</source>
        <translation>用户%1：不支持的命令类型</translation>
    </message>
    <message id="ship-refresh-failed">
        <location filename="../Server/server.cpp" line="3962"/>
        <source>Load ship table failed!</source>
        <translation>加载舰船列表失败！</translation>
    </message>
    <message id="ship-load-good">
        <location filename="../Server/server.cpp" line="3977"/>
        <source>Load ship registry success!</source>
        <translation>加载舰船列表成功！</translation>
    </message>
    <message id="factory-state-error">
        <location filename="../Server/server.cpp" line="4010"/>
        <source>Open user %1&apos;s factory failed!</source>
        <translation>打开用户%1的工厂槽失败！</translation>
    </message>
    <message id="delete-equip-nonexistent">
        <location filename="../Server/server.cpp" line="4046"/>
        <source>User id %1: equipment %2 does not exist when destructing!</source>
        <oldsource>User id %1: equipment %2 does not exist!</oldsource>
        <translation type="unfinished">用户ID%1：装备%2不存在！</translation>
    </message>
    <message id="delete-equip-failed">
        <location filename="../Server/server.cpp" line="4067"/>
        <source>User id %1: delete equipment failed!</source>
        <translation>用户ID%1：废弃装备失败！</translation>
    </message>
    <message id="delete-equip">
        <location filename="../Server/server.cpp" line="4074"/>
        <source>User id %1: deleted equipment %2</source>
        <translation>用户ID%1：废弃装备%2</translation>
    </message>
    <message id="open-db-failed">
        <location filename="../Server/server.cpp" line="4098"/>
        <source>Open database failed!</source>
        <oldsource>User id %1: deleted equipment %2</oldsource>
        <translation>数据库打开失败！</translation>
    </message>
    <message id="sql-connect-success">
        <location filename="../Server/server.cpp" line="4102"/>
        <source>SQL connection successful!</source>
        <translation>数据库连接成功！</translation>
    </message>
    <message id="equip-db-lack">
        <location filename="../Server/server.cpp" line="4158"/>
        <source>Equipment database does not exist, creating...</source>
        <translation>创建装备数据库……</translation>
    </message>
    <message id="equip-db-gen-failure">
        <location filename="../Server/server.cpp" line="4163"/>
        <source>Create Equipment database failed.</source>
        <translation>创建装备数据库失败。</translation>
    </message>
    <message id="equip-name-db-lack">
        <location filename="../Server/server.cpp" line="4170"/>
        <source>Equipment name database does not exist, creating...</source>
        <translation>创建装备名数据库……</translation>
    </message>
    <message id="equip-name-db-gen-failure">
        <location filename="../Server/server.cpp" line="4175"/>
        <source>Create Equipment name database failed.</source>
        <translation>创建装备名数据库失败。</translation>
    </message>
    <message id="equip-sp-db-lack">
        <location filename="../Server/server.cpp" line="4182"/>
        <source>User equipment skillpoints database does not exist, creating...</source>
        <translation>创建装备技能点数据库……</translation>
    </message>
    <message id="equip-sp-db-gen-failure">
        <location filename="../Server/server.cpp" line="4187"/>
        <source>User equipment skillpoints fetch failure.</source>
        <oldsource>User equipment skillpoints fetch failure!</oldsource>
        <translation>创建装备技能点数据库失败。</translation>
    </message>
    <message id="equip-db-user-lack">
        <location filename="../Server/server.cpp" line="4194"/>
        <source>Equipment database for user does not exist, creating...</source>
        <translation>创建用户装备数据库……</translation>
    </message>
    <message id="equip-db-user-gen-failure">
        <location filename="../Server/server.cpp" line="4199"/>
        <source>Create Equipment database for user failed.</source>
        <translation>创建用户装备数据库失败。</translation>
    </message>
    <message id="equip-db-kc-user-lack">
        <location filename="../Server/server.cpp" line="4206"/>
        <source>Equipment database (kancolle) for user does not exist, creating...</source>
        <translation>创建用户装备数据库（舰C）……</translation>
    </message>
    <message id="equip-db-kc-user-gen-failure">
        <location filename="../Server/server.cpp" line="4212"/>
        <source>Create Equipment database (kancolle) for user failed.</source>
        <translation>创建用户装备数据库（舰C）失败。</translation>
    </message>
    <message id="facto-db-lack">
        <location filename="../Server/server.cpp" line="4219"/>
        <source>Factory database does not exist, creating...</source>
        <translation>创建工厂数据库……</translation>
    </message>
    <message id="facto-db-gen-failure">
        <location filename="../Server/server.cpp" line="4224"/>
        <source>Create Factory database failed.</source>
        <translation>创建工厂数据库失败。</translation>
    </message>
    <message id="map-node-db-lack">
        <location filename="../Server/server.cpp" line="4231"/>
        <source>Map node database does not exist, creating...</source>
        <translation>创建海域点数据库……</translation>
    </message>
    <message id="map-node-db-gen-failure">
        <location filename="../Server/server.cpp" line="4236"/>
        <source>Create Map node database failed.</source>
        <translation>创建海域点数据库失败。</translation>
    </message>
    <message id="map-relation-db-lack">
        <location filename="../Server/server.cpp" line="4243"/>
        <source>Map relation database does not exist, creating...</source>
        <translation>创建海域关系数据库……</translation>
    </message>
    <message id="map-relation-db-gen-failure">
        <location filename="../Server/server.cpp" line="4248"/>
        <source>Create Map relation database failed.</source>
        <translation>创建海域关系数据库失败。</translation>
    </message>
    <message id="map-resource-db-lack">
        <location filename="../Server/server.cpp" line="4255"/>
        <source>Map resource database does not exist, creating...</source>
        <translation>创建海域资源数据库……</translation>
    </message>
    <message id="map-resource-db-gen-failure">
        <location filename="../Server/server.cpp" line="4260"/>
        <source>Create Map resource database failed.</source>
        <translation>创建海域资源数据库失败。</translation>
    </message>
    <message id="ship-db-lack">
        <location filename="../Server/server.cpp" line="4267"/>
        <source>Ship database does not exist, creating...</source>
        <translation>创建舰船数据库……</translation>
    </message>
    <message id="equip-ship-db-gen-failure">
        <location filename="../Server/server.cpp" line="4272"/>
        <source>Create Ship database failed.</source>
        <translation>创建舰船数据库失败。</translation>
    </message>
    <message id="ship-name-db-lack">
        <location filename="../Server/server.cpp" line="4279"/>
        <source>Ship name database does not exist, creating...</source>
        <translation>创建舰船名数据库……</translation>
    </message>
    <message id="equip-ship-name-gen-failure">
        <location filename="../Server/server.cpp" line="4285"/>
        <source>Create Ship name failed.</source>
        <translation>创建舰船名数据库失败。</translation>
    </message>
    <message id="ship-db-user-lack">
        <location filename="../Server/server.cpp" line="4292"/>
        <source>Ship database for user does not exist, creating...</source>
        <translation>创建用户舰船数据库……</translation>
    </message>
    <message id="ship-db-user-gen-failure">
        <location filename="../Server/server.cpp" line="4297"/>
        <source>Create Ship database for user failed.</source>
        <translation>创建用户舰船数据库失败。</translation>
    </message>
    <message id="ship-db-bp-user-lack">
        <location filename="../Server/server.cpp" line="4304"/>
        <source>Ship blueprint database for user does not exist, creating...</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-db-bp-user-gen-failure">
        <location filename="../Server/server.cpp" line="4310"/>
        <source>Create Ship blueprint database for user failed.</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-db-kc-user-lack">
        <location filename="../Server/server.cpp" line="4317"/>
        <source>Ship database (kancolle) for user does not exist, creating...</source>
        <translation>创建用户舰船数据库（舰C）……</translation>
    </message>
    <message id="ship-db-kc-user-gen-failure">
        <location filename="../Server/server.cpp" line="4323"/>
        <source>Create Ship database (kancolle) for user failed.</source>
        <oldsource>Create Equipment database (kancolle) for user failed.</oldsource>
        <translation type="unfinished">创建用户舰船数据库（舰C）失败。</translation>
    </message>
    <message id="user-db-lack">
        <location filename="../Server/server.cpp" line="4330"/>
        <source>User database does not exist, creating...</source>
        <translation>创建用户数据库……</translation>
    </message>
    <message id="user-db-gen-failure">
        <location filename="../Server/server.cpp" line="4335"/>
        <source>Create User database failed.</source>
        <translation>创建用户数据库失败。</translation>
    </message>
    <message id="user-db-attr-lack">
        <location filename="../Server/server.cpp" line="4341"/>
        <source>User attributes database does not exist, creating...</source>
        <translation>创建用户属性数据库……</translation>
    </message>
    <message id="user-db-attr-gen-failure">
        <location filename="../Server/server.cpp" line="4346"/>
        <source>Create User attributes database failed.</source>
        <translation>创建用户属性数据库失败。</translation>
    </message>
    <message id="server-pem">
        <location filename="../Server/server.cpp" line="4365"/>
        <source>Server PEM is now %1.</source>
        <translation>服务器PEM现在是%1。</translation>
    </message>
    <message id="update-fleet-clear-failure">
        <location filename="../Server/server.cpp" line="4493"/>
        <source>Update fleet (clear fleet) failure!</source>
        <translation>更新舰队（清除舰队）失败！</translation>
    </message>
    <message id="update-fleet-failure">
        <location filename="../Server/server.cpp" line="4392"/>
        <location filename="../Server/server.cpp" line="4513"/>
        <location filename="../Server/server.cpp" line="4540"/>
        <location filename="../Server/server.cpp" line="4569"/>
        <location filename="../Server/server.cpp" line="4589"/>
        <source>Update fleet failure!</source>
        <translation>更新舰队失败！</translation>
    </message>
    <message id="ship-cant-equip-it">
        <location filename="../Server/server.cpp" line="4525"/>
        <source>Ship %1 can&apos;t equip %2!</source>
        <translation>舰船%1无法装备%2！</translation>
    </message>
    <message id="ship-cant-equip-it-extra">
        <location filename="../Server/server.cpp" line="4553"/>
        <source>Ship %1 can&apos;t equip %2 in extra slot!</source>
        <translation>舰船%1无法在补强格装备%2！</translation>
    </message>
    <message id="user-data-init-fail">
        <location filename="../Server/server.cpp" line="4624"/>
        <location filename="../Server/server.cpp" line="4644"/>
        <source>%1: User data init failure!</source>
        <translation>%1：用户数据初始化失败！</translation>
    </message>
    <message id="user-factory-init-fail">
        <location filename="../Server/server.cpp" line="4662"/>
        <source>Init 4 factory slots for user %1 failed!</source>
        <translation>%1：用户4个工厂槽初始化失败！</translation>
    </message>
    <message id="init-userfleet-failed">
        <location filename="../Server/server.cpp" line="1017"/>
        <location filename="../Server/server.cpp" line="4682"/>
        <source>Set User Fleet Up failed!</source>
        <translation>初始化用户舰队失败！</translation>
    </message>
    <message id="sender-error">
        <location filename="../Server/servermastersender.cpp" line="36"/>
        <source>Address %1 Port %2 Error: %3</source>
        <oldsource>Address %1 Port %2 Errror: %3</oldsource>
        <translation>地址：%1，端口：%2，错误：%3</translation>
    </message>
    <message id="user-add-skillpoint-failed">
        <location filename="../Server/user.cpp" line="68"/>
        <source>User %1: add skill point to equipment id %2 failed!</source>
        <translation>用户%1：向装备%2添加技能点失败！</translation>
    </message>
    <message id="user-add-skillpoint-success">
        <location filename="../Server/user.cpp" line="74"/>
        <source>User %1: add skillpoint of equipment %2 success, result: %3</source>
        <translation>用户%1：向装备%2添加技能点成功，点数：%3</translation>
    </message>
    <message id="user-add-homeport-failure">
        <location filename="../Server/user.cpp" line="107"/>
        <source>User %1: set home port failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="user-add-homeport-success">
        <location filename="../Server/user.cpp" line="113"/>
        <source>User %1: set home port</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="user-get-factory-developing-failed">
        <location filename="../Server/user.cpp" line="129"/>
        <source>User %1: get num of factory currently developing equipment %2 failed!</source>
        <translation>用户%1：获取目前正开发装备%2的工厂槽数失败！</translation>
    </message>
    <message id="user-check-resource-failed">
        <location filename="../Server/user.cpp" line="159"/>
        <source>User %1: check resources failed!</source>
        <translation>用户%1：检查资源失败！</translation>
    </message>
    <message id="user-get-equip-amount-failed">
        <location filename="../Server/user.cpp" line="195"/>
        <source>User %1: get equipment amount of %2 failed!</source>
        <translation>用户%1：获取装备%2的数量失败！</translation>
    </message>
    <message id="user-get-equip-data-failed">
        <location filename="../Server/user.cpp" line="218"/>
        <source>Get equipment data of %1 failed!</source>
        <translation>获取装备%1的数据失败！</translation>
    </message>
    <message id="user-get-ship-data-failed">
        <location filename="../Server/user.cpp" line="240"/>
        <source>Get ship data of %1 failed!</source>
        <translation>获取舰船%1的数据失败！</translation>
    </message>
    <message id="user-get-skillpoint-failed">
        <location filename="../Server/user.cpp" line="263"/>
        <source>User %1: get skill point of equipment %2 failed!</source>
        <translation>用户%1：获取装备%2的技能点失败！</translation>
    </message>
    <message id="init-userfactory-failed">
        <location filename="../Server/user.cpp" line="361"/>
        <source>Set User Factory Up failed!</source>
        <translation>初始化工厂槽失败！</translation>
    </message>
    <message id="user-nonexistent-uid">
        <location filename="../Server/user.cpp" line="379"/>
        <location filename="../Server/user.cpp" line="400"/>
        <location filename="../Server/user.cpp" line="421"/>
        <source>User ID %1 does not exist!</source>
        <translation>用户%1不存在！</translation>
    </message>
    <message id="new-ship-failed">
        <location filename="../Server/user.cpp" line="486"/>
        <source>User id %1: new ship failed!</source>
        <translation>用户%1：建造舰船失败！</translation>
    </message>
    <message id="new-ship">
        <location filename="../Server/user.cpp" line="493"/>
        <source>User id %1: new ship %2 definition %3</source>
        <translation>用户%1：建造舰船序列号%2，船名ID%3</translation>
    </message>
    <message id="new-equip-failed">
        <location filename="../Server/user.cpp" line="450"/>
        <source>User id %1: new equipment failed!</source>
        <translation>用户%1：开发装备失败！</translation>
    </message>
    <message id="user-add-ship-bp-failed">
        <location filename="../Server/user.cpp" line="41"/>
        <source>User %1: add blueprint of ship %2 failed!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="user-add-ship-bp-success">
        <location filename="../Server/user.cpp" line="47"/>
        <source>User %1: add blueprint of ship %2 success!</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="new-equip">
        <location filename="../Server/user.cpp" line="457"/>
        <source>User id %1: new equipment %2 definition %3</source>
        <translation>用户%1：开发装备序列号%2，装备名ID%3</translation>
    </message>
    <message id="dbfail-when-refresh-factory">
        <location filename="../Server/user.cpp" line="509"/>
        <source>User ID %1: DB failure when refreshing factory</source>
        <translation>用户%1：更新工厂时发生数据库错误</translation>
    </message>
    <message id="set-resources-failed">
        <location filename="../Server/user.cpp" line="550"/>
        <source>User id %1: set resources failed!</source>
        <translation>用户%1：更新资源失败！</translation>
    </message>
    <message id="set-resources">
        <location filename="../Server/user.cpp" line="556"/>
        <source>User id %1: set resources</source>
        <translation>用户%1：更新资源</translation>
    </message>
    <message id="equip-attr-tech">
        <location filename="../Protocol/equipment.h" line="25"/>
        <source>Tech</source>
        <oldsource>Technology</oldsource>
        <translation>科技等级</translation>
    </message>
    <message id="equip-attr-father">
        <location filename="../Protocol/equipment.h" line="27"/>
        <source>Precedent</source>
        <translation>装备前置</translation>
    </message>
    <message id="equip-attr-father2">
        <location filename="../Protocol/equipment.h" line="29"/>
        <source>Precedent 2</source>
        <translation>装备前置</translation>
    </message>
    <message id="equip-attr-mother">
        <location filename="../Protocol/equipment.h" line="31"/>
        <source>Skill points pool</source>
        <translation>装备技能来源</translation>
    </message>
    <message id="equip-attr-disallowmassproduction">
        <location filename="../Protocol/equipment.h" line="33"/>
        <source>Possessing limit</source>
        <translation>持有上限</translation>
    </message>
    <message id="equip-attr-hitpoints">
        <location filename="../Protocol/equipment.h" line="35"/>
        <source>Hitpoints</source>
        <translation>耐久</translation>
    </message>
    <message id="equip-attr-firepower">
        <location filename="../Protocol/equipment.h" line="37"/>
        <source>Firepower</source>
        <translation>火力</translation>
    </message>
    <message id="equip-attr-armor">
        <location filename="../Protocol/equipment.h" line="39"/>
        <source>Armor</source>
        <translation>装甲</translation>
    </message>
    <message id="equip-attr-armorpenetration">
        <location filename="../Protocol/equipment.h" line="41"/>
        <source>AP</source>
        <translation>穿甲</translation>
    </message>
    <message id="equip-attr-accuracy">
        <location filename="../Protocol/equipment.h" line="43"/>
        <source>Accuracy</source>
        <translation>命中</translation>
    </message>
    <message id="equip-attr-torpedoaccuracy">
        <location filename="../Protocol/equipment.h" line="45"/>
        <source>Accuracy(torp)</source>
        <translation>鱼类命中</translation>
    </message>
    <message id="equip-attr-evasion">
        <location filename="../Protocol/equipment.h" line="47"/>
        <source>Evasion</source>
        <translation>回避</translation>
    </message>
    <message id="equip-attr-los">
        <location filename="../Protocol/equipment.h" line="49"/>
        <source>LOS</source>
        <translation>索敌</translation>
    </message>
    <message id="equip-attr-concealment">
        <location filename="../Protocol/equipment.h" line="51"/>
        <source>Concealment</source>
        <translation>隐蔽</translation>
    </message>
    <message id="equip-attr-firingrange">
        <location filename="../Protocol/equipment.h" line="53"/>
        <source>Firing range</source>
        <translation>射程</translation>
    </message>
    <message id="equip-attr-firingspeed">
        <location filename="../Protocol/equipment.h" line="55"/>
        <source>Firing speed</source>
        <translation>射速</translation>
    </message>
    <message id="equip-attr-speed">
        <location filename="../Protocol/equipment.h" line="57"/>
        <source>Ship speed</source>
        <translation>船速</translation>
    </message>
    <message id="equip-attr-torpedo">
        <location filename="../Protocol/equipment.h" line="59"/>
        <source>Torpedo</source>
        <translation>雷装</translation>
    </message>
    <message id="equip-attr-airtorpedo">
        <location filename="../Protocol/equipment.h" line="61"/>
        <source>Torpedo(air)</source>
        <translation>航空雷装</translation>
    </message>
    <message id="equip-attr-bombing">
        <location filename="../Protocol/equipment.h" line="63"/>
        <source>Bombing</source>
        <translation>爆装</translation>
    </message>
    <message id="equip-attr-antiair">
        <location filename="../Protocol/equipment.h" line="65"/>
        <source>Anti-air</source>
        <translation>对空</translation>
    </message>
    <message id="equip-attr-asw">
        <location filename="../Protocol/equipment.h" line="67"/>
        <source>ASW</source>
        <translation>对潜</translation>
    </message>
    <message id="equip-attr-interception">
        <location filename="../Protocol/equipment.h" line="69"/>
        <source>Interception</source>
        <translation>迎击</translation>
    </message>
    <message id="equip-attr-antibomber">
        <location filename="../Protocol/equipment.h" line="71"/>
        <source>Anti-bomber</source>
        <oldsource>Anti bomber</oldsource>
        <translation>对爆</translation>
    </message>
    <message id="equip-attr-antiland">
        <location filename="../Protocol/equipment.h" line="73"/>
        <source>Anti-land</source>
        <oldsource>Anti land</oldsource>
        <translation>对地</translation>
    </message>
    <message id="equip-attr-transport">
        <location filename="../Protocol/equipment.h" line="75"/>
        <source>Transport</source>
        <translation>运输</translation>
    </message>
    <message id="equip-attr-flightrange">
        <location filename="../Protocol/equipment.h" line="77"/>
        <source>Flight range</source>
        <translation>航程</translation>
    </message>
    <message id="develop-success-rate-unknown">
        <location filename="../ClientGUI/ui/factory/developwindow.cpp" line="129"/>
        <source>Unknown</source>
        <translation>未知</translation>
    </message>
    <message id="diff-c">
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="45"/>
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="82"/>
        <source>Early</source>
        <translation>丙</translation>
    </message>
    <message id="diff-b">
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="47"/>
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="85"/>
        <source>Medium</source>
        <translation>乙</translation>
    </message>
    <message id="diff-a">
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="49"/>
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="88"/>
        <source>Late</source>
        <translation>甲</translation>
    </message>
    <message id="diff-s">
        <location filename="../ClientGUI/ui/sortie/sortie.cpp" line="92"/>
        <source>Historical</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-local-name-lack">
        <location filename="../Protocol/ship.cpp" line="28"/>
        <location filename="../Protocol/ship.cpp" line="45"/>
        <location filename="../Protocol/ship.cpp" line="62"/>
        <source>Local language (%1) for ship name not found!</source>
        <translation>语言%1的舰船名未找到！</translation>
    </message>
    <message id="ship-attr-lack">
        <location filename="../Protocol/ship.cpp" line="78"/>
        <source>Fetch ship attributes failure!</source>
        <translation>获取舰船属性失败！</translation>
    </message>
    <message id="remodel-nonexistent">
        <location filename="../Protocol/ship.cpp" line="170"/>
        <source>Remodel target %1 does not exist!</source>
        <translation>改造标的%1不存在！</translation>
    </message>
    <message id="all-shiptypes">
        <location filename="../ClientGUI/model/shipbpmodel.cpp" line="244"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="44"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="52"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="89"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="126"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="169"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="131"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="143"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="189"/>
        <source>All ship types</source>
        <translation>所有舰船</translation>
    </message>
    <message id="all-shipclasses">
        <location filename="../ClientGUI/model/shipbpmodel.cpp" line="246"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="46"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="56"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="91"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="146"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="182"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="146"/>
        <location filename="../ClientGUI/ui/views/shipselect.cpp" line="227"/>
        <source>All ship classes</source>
        <translation>所有舰级</translation>
    </message>
    <message id="fleet-idle">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="416"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="313"/>
        <source>Idle</source>
        <translation>空闲</translation>
    </message>
    <message id="ship-uuid">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="343"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="482"/>
        <location filename="../ClientGUI/model/specshipmodel.cpp" line="59"/>
        <source>UUID</source>
        <translation>序列号</translation>
    </message>
    <message id="ship-name">
        <location filename="../ClientGUI/model/shipbpmodel.cpp" line="140"/>
        <location filename="../ClientGUI/model/shipbpmodel.cpp" line="201"/>
        <location filename="../ClientGUI/model/shipdefmodel.cpp" line="54"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="347"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="485"/>
        <source>Name</source>
        <translation>舰船名</translation>
    </message>
    <message id="bp-amount">
        <location filename="../ClientGUI/model/shipbpmodel.cpp" line="143"/>
        <location filename="../ClientGUI/model/shipbpmodel.cpp" line="205"/>
        <source>Amount</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-star-tooltip">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="269"/>
        <source>Current Star %1, maximum star %2</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="fleet-disabled">
        <location filename="../ClientGUI/model/equipmodel.cpp" line="419"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="317"/>
        <location filename="../ClientGUI/ui/views/selectdelegate.cpp" line="86"/>
        <source>Disabled</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-star">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="351"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="489"/>
        <source>Modernization</source>
        <translation type="unfinished">近代化星级</translation>
    </message>
    <message id="ship-improve">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="355"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="492"/>
        <source>Modernize</source>
        <translation>近代化改造</translation>
    </message>
    <message id="ship-select">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="359"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="495"/>
        <source>Select this</source>
        <oldsource>Select</oldsource>
        <translation>选择此船</translation>
    </message>
    <message id="ship-hp">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="363"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="498"/>
        <source>HP</source>
        <translation>耐久</translation>
    </message>
    <message id="ship-cond">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="367"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="501"/>
        <source>Cond.</source>
        <translation>状态</translation>
    </message>
    <message id="ship-lv">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="371"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="504"/>
        <source>Level</source>
        <translation>等级</translation>
    </message>
    <message id="ship-pos">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="375"/>
        <location filename="../ClientGUI/model/shipmodel.cpp" line="507"/>
        <source>Position</source>
        <translation>舰队位置</translation>
    </message>
    <message id="ship-star-whatsthis">
        <location filename="../ClientGUI/model/shipmodel.cpp" line="470"/>
        <source>Modernization/Maximum Modernization</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="menu-develop">
        <source>Develop Equipment</source>
        <translation type="obsolete">装备开发</translation>
    </message>
    <message id="bytes-read-total">
        <location filename="../ClientGUI/resourcefetch.cpp" line="21"/>
        <source>Read %1 of %2 bytes</source>
        <translation>共%2字节，读取了%1字节</translation>
    </message>
    <message id="url-invalid">
        <location filename="../ClientGUI/resourcefetch.cpp" line="58"/>
        <source>Invalid URL: %1: %2</source>
        <translation>非法URL: %1，错误%2</translation>
    </message>
    <message id="unable-to-save-file">
        <location filename="../ClientGUI/resourcefetch.cpp" line="88"/>
        <source>Unable to save the file %1: %2.</source>
        <translation>未能保存入文件%1：错误%2。</translation>
    </message>
    <message id="download-failed">
        <location filename="../ClientGUI/resourcefetch.cpp" line="122"/>
        <source>Download failed: %1</source>
        <translation>下载失败，错误：%1</translation>
    </message>
    <message id="download-success">
        <location filename="../ClientGUI/resourcefetch.cpp" line="130"/>
        <source>Downloaded %1 bytes to %2 in %3</source>
        <translation>下载了%1字节至文件夹%3下的文件%2</translation>
    </message>
    <message id="ssl-errors">
        <location filename="../ClientGUI/resourcefetch.cpp" line="160"/>
        <source>One or more TLS errors has occurred: %1</source>
        <translation>发生了一个或多个TLS错误：%1</translation>
    </message>
    <message id="fleet-pos-head">
        <location filename="../ClientGUI/ui/fleet/fleetview.cpp" line="38"/>
        <source>Pos</source>
        <translation>位置</translation>
    </message>
    <message id="fleetview-equip-slot-ex">
        <location filename="../ClientGUI/ui/fleet/fleetview.cpp" line="96"/>
        <source>Equip Ex</source>
        <translation>补强装备栏</translation>
    </message>
    <message id="fleetview-equip-slot">
        <location filename="../ClientGUI/ui/fleet/fleetview.cpp" line="100"/>
        <source>Equip %1</source>
        <oldsource>Equip Ex</oldsource>
        <translation>装备栏%1</translation>
    </message>
    <message id="fleetview-view-ship-attr">
        <location filename="../ClientGUI/ui/fleet/fleetview.cpp" line="121"/>
        <source>Attributes</source>
        <translation>属性</translation>
    </message>
    <message id="ship-name-head">
        <source></source>
        <oldsource>Ship Name</oldsource>
        <translation type="obsolete">无</translation>
    </message>
    <message id="fleet-no-ship">
        <source></source>
        <oldsource>None</oldsource>
        <translation type="obsolete">无</translation>
    </message>
    <message id="Japanese">
        <location filename="../Protocol/kp.h" line="350"/>
        <source>Japanese</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="German">
        <location filename="../Protocol/kp.h" line="352"/>
        <source>German</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Italian">
        <location filename="../Protocol/kp.h" line="354"/>
        <source>Italian</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="American">
        <location filename="../Protocol/kp.h" line="356"/>
        <source>American</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="British">
        <location filename="../Protocol/kp.h" line="358"/>
        <source>British</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="French">
        <location filename="../Protocol/kp.h" line="360"/>
        <source>French</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Soviet">
        <location filename="../Protocol/kp.h" line="362"/>
        <source>Soviet</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Chinese">
        <location filename="../Protocol/kp.h" line="364"/>
        <source>Chinese</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Benelux">
        <location filename="../Protocol/kp.h" line="366"/>
        <source>Benelux</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Nordic">
        <location filename="../Protocol/kp.h" line="368"/>
        <source>Nordic</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Commonwealth">
        <location filename="../Protocol/kp.h" line="370"/>
        <source>Commonwealth</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Latin">
        <location filename="../Protocol/kp.h" line="372"/>
        <source>Iberian/Latin American</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="EasternEuropean">
        <location filename="../Protocol/kp.h" line="374"/>
        <source>Eastern European</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="MinorAsian">
        <location filename="../Protocol/kp.h" line="376"/>
        <source>Other Asian</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="Fantasy">
        <location filename="../Protocol/kp.h" line="378"/>
        <source>Fantasy ships</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="NormalFleet">
        <location filename="../Protocol/kp.h" line="445"/>
        <source>Normal</source>
        <translation>通常舰队</translation>
    </message>
    <message id="CarrierFleet">
        <location filename="../Protocol/kp.h" line="447"/>
        <source>Carrier</source>
        <translation>空母机动</translation>
    </message>
    <message id="SurfaceFleet">
        <location filename="../Protocol/kp.h" line="449"/>
        <source>Surface</source>
        <translation>水上打击</translation>
    </message>
    <message id="TransportFleet">
        <location filename="../Protocol/kp.h" line="451"/>
        <source>Transport</source>
        <translation>输送护卫</translation>
    </message>
    <message id="lv-display">
        <location filename="../ClientGUI/ui/fleet/shipdisplay.cpp" line="56"/>
        <source>Lv %1</source>
        <translation>Lv %1</translation>
    </message>
    <message id="blueprintview">
        <location filename="../ClientGUI/ui/factory/factoryarea.cpp" line="185"/>
        <source>Blueprints</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="force-fetch-question">
        <location filename="../ClientGUI/ui/factory/factoryarea.cpp" line="233"/>
        <source>Do you really want to spend resources to speed up this slot?</source>
        <oldsource>Do you really want to spend resources to speed up thi slot?</oldsource>
        <translation type="unfinished"></translation>
    </message>
    <message id="o-expr">
        <location filename="../Protocol/resord.cpp" line="38"/>
        <source>%1 Oil </source>
        <translation type="unfinished">%1油</translation>
    </message>
    <message id="e-expr">
        <location filename="../Protocol/resord.cpp" line="40"/>
        <source>%1 Ammo </source>
        <translation type="unfinished">%1弹</translation>
    </message>
    <message id="s-expr">
        <location filename="../Protocol/resord.cpp" line="42"/>
        <source>%1 Steel </source>
        <translation type="unfinished">%1钢</translation>
    </message>
    <message id="r-expr">
        <location filename="../Protocol/resord.cpp" line="44"/>
        <source>%1 Rubber </source>
        <translation type="unfinished">%1橡胶</translation>
    </message>
    <message id="a-expr">
        <location filename="../Protocol/resord.cpp" line="46"/>
        <source>%1 Aluminum </source>
        <translation type="unfinished">%1铝</translation>
    </message>
    <message id="w-expr">
        <location filename="../Protocol/resord.cpp" line="48"/>
        <source>%1 Tungsten </source>
        <translation type="unfinished">%1钨</translation>
    </message>
    <message id="c-expr">
        <location filename="../Protocol/resord.cpp" line="50"/>
        <source>%1 Chromium </source>
        <translation type="unfinished">%1铬</translation>
    </message>
    <message id="o-expr-s">
        <location filename="../Protocol/resord.cpp" line="55"/>
        <source>%1O</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="e-expr-s">
        <location filename="../Protocol/resord.cpp" line="57"/>
        <source>%1E</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="s-expr-s">
        <location filename="../Protocol/resord.cpp" line="59"/>
        <source>%1S</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="r-expr-s">
        <location filename="../Protocol/resord.cpp" line="61"/>
        <source>%1R</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="a-expr-s">
        <location filename="../Protocol/resord.cpp" line="63"/>
        <source>%1Al</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="w-expr-s">
        <location filename="../Protocol/resord.cpp" line="65"/>
        <source>%1W</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="c-expr-s">
        <location filename="../Protocol/resord.cpp" line="67"/>
        <source>%1Cr</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="settings-key">
        <location filename="../ClientGUI/ui/settingswindow.cpp" line="26"/>
        <source>Key</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="settings-value">
        <location filename="../ClientGUI/ui/settingswindow.cpp" line="28"/>
        <source>Value</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>ChooseHomePort</name>
    <message>
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="14"/>
        <source>Dialog</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="choose-home">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="37"/>
        <source>Choose your home port</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-japan">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="49"/>
        <source>Seto Inland Sea</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-britain">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="56"/>
        <source>The Channel</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-germany">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="63"/>
        <source>Kronborg</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-france">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="70"/>
        <source>La Manche</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-italy">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="77"/>
        <source>Sicily</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-soviet">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="84"/>
        <source>Gulf of Finland</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-usa">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="91"/>
        <source>Eastern Seaboard</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="home-australia">
        <location filename="../ClientGUI/ui/port/choosehomeport.ui" line="98"/>
        <source>East Australia</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>ConstructWindow</name>
    <message>
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="14"/>
        <source>Dialog</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="select-ship-nation">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="34"/>
        <source>Select Ship Nationality:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="default-equip-1">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="66"/>
        <source>Default Equipment 1:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="default-equip-none">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="85"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="168"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="251"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="334"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="423"/>
        <location filename="../ClientGUI/ui/factory/constructwindow.cpp" line="246"/>
        <source>(Not required)</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="select-ship-type">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="117"/>
        <source>Select Ship Type:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="default-equip-2">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="149"/>
        <source>Default Equipment 2:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="select-ship-class">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="200"/>
        <source>Select Ship Class:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="default-equip-3">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="232"/>
        <source>Default Equipment 3:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="select-ship-name">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="283"/>
        <source>Select Ship Name:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="default-equip-4">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="315"/>
        <source>Default Equipment 4:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="or-enter-ship-id">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="366"/>
        <source>Or Enter Ship ID:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="default-equip-5">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="404"/>
        <source>Default Equipment 5:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="ship-remodel">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="459"/>
        <source>Ship to Remodel:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="cons-resource-required">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="489"/>
        <source>Resource required:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="dev-calculating">
        <location filename="../ClientGUI/ui/factory/constructwindow.ui" line="502"/>
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="112"/>
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="152"/>
        <source>Calculating...</source>
        <translation type="unfinished">正在计算……</translation>
    </message>
</context>
<context>
    <name>DevelopWindow</name>
    <message id="or-enter-equipment-id">
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="88"/>
        <source>Or Enter Equipment ID:</source>
        <oldsource>Or Enter Ship ID:</oldsource>
        <translation type="unfinished">或键入装备ID：</translation>
    </message>
    <message id="select-equipment-name">
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="50"/>
        <source>Select Equipment Name:</source>
        <translation>选择装备名：</translation>
    </message>
    <message id="select-equipment-type">
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="69"/>
        <source>Select Equipment Type:</source>
        <translation>选择装备类型：</translation>
    </message>
    <message id="dev-success-rate">
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="105"/>
        <source>Estimated success rate:</source>
        <translation>估计成功率：</translation>
    </message>
    <message id="dev-resource-requied">
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="136"/>
        <source>Resource required:</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="develop-calculate">
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="119"/>
        <source>Calculate</source>
        <translation>计算</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/developwindow.ui" line="14"/>
        <source>Develop Equipment</source>
        <translation>装备开发</translation>
    </message>
</context>
<context>
    <name>EquipView</name>
    <message>
        <location filename="../ClientGUI/ui/views/equipview.ui" line="14"/>
        <source>Form</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>FactoryArea</name>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="14"/>
        <source>Frame</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="34"/>
        <source>Equipment Development</source>
        <translation>装备开发</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="94"/>
        <source>Factory 0</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="101"/>
        <source>Factory 6</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="108"/>
        <source>Factory 12</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="115"/>
        <source>Factory 18</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="122"/>
        <source>Factory 1</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="129"/>
        <source>Factory 7</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="136"/>
        <source>Factory 13</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="143"/>
        <source>Factory 19</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="150"/>
        <source>Factory 2</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="157"/>
        <source>Factory 8</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="164"/>
        <source>Factory 14</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="171"/>
        <source>Factory 20</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="178"/>
        <source>Factory 3</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="185"/>
        <source>Factory 9</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="192"/>
        <source>Factory 15</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="199"/>
        <source>Factory 21</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="206"/>
        <source>Factory 4</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="213"/>
        <source>Factory 10</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="220"/>
        <source>Factory 16</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="227"/>
        <source>Factory 22</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="234"/>
        <source>Factory 5</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="241"/>
        <source>Factory 11</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="248"/>
        <source>Factory 17</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/factory/factoryarea.ui" line="255"/>
        <source>Factory 23</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>FleetView</name>
    <message id="load-fleet-wait">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="245"/>
        <source>Loading Fleets, please wait...</source>
        <translation>加载舰队中，请等待……</translation>
    </message>
    <message id="fleetview-fleet">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="81"/>
        <source>Fleet:</source>
        <translation>舰队：</translation>
    </message>
    <message id="fleetview-fleet1">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="106"/>
        <source>1</source>
        <translation>1</translation>
    </message>
    <message id="fleetview-fleet2">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="131"/>
        <source>2</source>
        <translation>2</translation>
    </message>
    <message id="fleetview-fleet3">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="156"/>
        <source>3</source>
        <translation>3</translation>
    </message>
    <message id="fleetview-fleet4">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="181"/>
        <source>4</source>
        <translation>4</translation>
    </message>
    <message id="fleetview-fleetexp">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="188"/>
        <source>Expedition Fleet:</source>
        <translation>远征舰队：</translation>
    </message>
    <message id="fleetview-fleet-type">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="198"/>
        <source>Fleet type:</source>
        <translation>舰队类型：</translation>
    </message>
    <message id="fleet-save">
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="208"/>
        <source>Save</source>
        <translation>保存</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/fleet/fleetview.ui" line="14"/>
        <source>Form</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>LicenseArea</name>
    <message>
        <location filename="../ClientGUI/ui/port/licensearea.ui" line="20"/>
        <source>Frame</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/port/licensearea.ui" line="115"/>
        <source>Continue</source>
        <extracomment>License Continue</extracomment>
        <translation>继续游戏</translation>
    </message>
</context>
<context>
    <name>MainWindow</name>
    <message id="develop-equipment">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="424"/>
        <location filename="../ClientGUI/ui/factory/factoryarea.cpp" line="155"/>
        <source>Develop Equipment</source>
        <oldsource>Develop equipment</oldsource>
        <translation>装备开发</translation>
    </message>
    <message id="construct-ships">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="449"/>
        <location filename="../ClientGUI/ui/factory/factoryarea.cpp" line="160"/>
        <source>Construct Ships</source>
        <oldsource>Consturct Ships</oldsource>
        <translation>舰船建造</translation>
    </message>
    <message id="arsenal">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="454"/>
        <location filename="../ClientGUI/ui/factory/factoryarea.cpp" line="166"/>
        <source>Arsenal</source>
        <translation>军械库</translation>
    </message>
    <message id="anchorage">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="464"/>
        <location filename="../ClientGUI/ui/factory/factoryarea.cpp" line="175"/>
        <source>Anchorage</source>
        <translation>锚地</translation>
    </message>
    <message id="mainwindow-title">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="14"/>
        <source>FleetMemories</source>
        <translation>舰队记忆</translation>
    </message>
    <message id="menu-sortie">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="366"/>
        <source>Sortie</source>
        <translation>出击</translation>
    </message>
    <message id="menu-fleet">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="372"/>
        <source>Fleet</source>
        <translation>编成</translation>
    </message>
    <message id="menu-dock">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="378"/>
        <source>Dock</source>
        <translation>入渠</translation>
    </message>
    <message id="menu-factory">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="383"/>
        <source>Factory</source>
        <translation>工厂</translation>
    </message>
    <message id="menu-naval-base">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="393"/>
        <source>Naval Base</source>
        <translation>母港</translation>
    </message>
    <message id="menu-tech">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="401"/>
        <source>Tech</source>
        <translation>科技</translation>
    </message>
    <message id="menu-help">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="407"/>
        <source>Help</source>
        <translation type="unfinished">帮助</translation>
    </message>
    <message id="menu-back-to-naval-base">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="429"/>
        <source>Back to naval base</source>
        <translation>返回母港</translation>
    </message>
    <message id="menu-logout">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="434"/>
        <source>Logout</source>
        <translation>登出</translation>
    </message>
    <message id="menu-exit">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="439"/>
        <source>Exit program</source>
        <translation>退出程序</translation>
    </message>
    <message id="menu-view-tech">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="444"/>
        <source>View Tech</source>
        <translation>查看科技</translation>
    </message>
    <message id="menu-battle">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="459"/>
        <source>Battle Maps</source>
        <translation>战斗海域一览</translation>
    </message>
    <message id="menu-compose">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="469"/>
        <source>Compose</source>
        <translation>编成</translation>
    </message>
    <message id="ship-blueprint-view">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="474"/>
        <source>Ship Blueprints</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="menu-settings">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="479"/>
        <source>Settings</source>
        <translation type="unfinished">设置</translation>
    </message>
    <message id="menu-about-qt">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="484"/>
        <source>About Qt</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="menu-license">
        <location filename="../ClientGUI/ui/mainwindow.ui" line="489"/>
        <source>License</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="69"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="76"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="83"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="90"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="97"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="104"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="111"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="118"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="125"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="132"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="139"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="146"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="153"/>
        <location filename="../ClientGUI/ui/mainwindow.ui" line="160"/>
        <source>TextLabel</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>NewLoginS</name>
    <message id="login-options-server">
        <location filename="../ClientGUI/ui/port/newlogins.ui" line="58"/>
        <source>Server:</source>
        <translation>服务器：</translation>
    </message>
    <message id="login-options-port">
        <location filename="../ClientGUI/ui/port/newlogins.ui" line="84"/>
        <source>Port:</source>
        <translation>端口：</translation>
    </message>
    <message id="login-options-connect">
        <location filename="../ClientGUI/ui/port/newlogins.ui" line="104"/>
        <source>Connect</source>
        <translation>连接</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/port/newlogins.ui" line="14"/>
        <source>Frame</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>PortArea</name>
    <message>
        <location filename="../ClientGUI/ui/port/portarea.ui" line="23"/>
        <source>PortArea</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/port/portarea.ui" line="49"/>
        <source>Hello!</source>
        <translation>欢迎！</translation>
    </message>
</context>
<context>
    <name>SettingsWindow</name>
    <message>
        <location filename="../ClientGUI/ui/settingswindow.ui" line="14"/>
        <source>Dialog</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="language-label">
        <location filename="../ClientGUI/ui/settingswindow.ui" line="22"/>
        <source>Language (need restart):</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="settings-head">
        <location filename="../ClientGUI/ui/settingswindow.ui" line="34"/>
        <source>Client settings registry</source>
        <translation type="unfinished"></translation>
    </message>
</context>
<context>
    <name>ShipDisplay</name>
    <message>
        <location filename="../ClientGUI/ui/fleet/shipdisplay.ui" line="14"/>
        <source>Form</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/fleet/shipdisplay.ui" line="72"/>
        <source>%v/%m</source>
        <translation>%v/%m</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/fleet/shipdisplay.ui" line="79"/>
        <source>Te</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/fleet/shipdisplay.ui" line="96"/>
        <source>Lv 0</source>
        <translation>Lv 0</translation>
    </message>
</context>
<context>
    <name>ShipEquip</name>
    <message>
        <location filename="../ClientGUI/ui/fleet/shipequip.ui" line="14"/>
        <source>Form</source>
        <translation type="unfinished"></translation>
    </message>
    <message id="empty-equip-slot">
        <location filename="../ClientGUI/ui/fleet/shipequip.ui" line="93"/>
        <location filename="../ClientGUI/ui/fleet/shipequip.cpp" line="149"/>
        <source>Empty</source>
        <translation>空装备格</translation>
    </message>
</context>
<context>
    <name>Sortie</name>
    <message>
        <location filename="../ClientGUI/ui/sortie/sortie.ui" line="14"/>
        <source>Form</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/sortie/sortie.ui" line="75"/>
        <source>Difficulty:</source>
        <extracomment>map-diff</extracomment>
        <translation>难度：</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/sortie/sortie.ui" line="105"/>
        <source>Selected Map:</source>
        <extracomment>selected-map</extracomment>
        <translation>已选择的海域：</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/sortie/sortie.ui" line="112"/>
        <source>None</source>
        <extracomment>selected-map-id</extracomment>
        <translation>无</translation>
    </message>
</context>
<context>
    <name>TechView</name>
    <message id="techview-globaltech">
        <location filename="../ClientGUI/ui/techview.ui" line="24"/>
        <source>GlobalTech:</source>
        <translation>全局科技：</translation>
    </message>
    <message id="techview-calculating">
        <location filename="../ClientGUI/ui/techview.ui" line="34"/>
        <source>Calculating...</source>
        <translation>正在计算……</translation>
    </message>
    <message id="techview-update">
        <location filename="../ClientGUI/ui/techview.ui" line="83"/>
        <source>Update</source>
        <translation type="unfinished">更新</translation>
    </message>
    <message id="techview-localtech">
        <location filename="../ClientGUI/ui/techview.ui" line="96"/>
        <source>LocalTech:</source>
        <translation>局部科技：</translation>
    </message>
    <message id="techview-na">
        <location filename="../ClientGUI/ui/techview.ui" line="106"/>
        <location filename="../ClientGUI/ui/techview.ui" line="130"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="111"/>
        <source>N/A</source>
        <translation>不适用</translation>
    </message>
    <message id="techview-skillpoints">
        <location filename="../ClientGUI/ui/techview.ui" line="120"/>
        <source>Skill Points:</source>
        <translation>技能点：</translation>
    </message>
    <message id="techview-toship">
        <location filename="../ClientGUI/ui/techview.ui" line="149"/>
        <location filename="../ClientGUI/ui/techview.cpp" line="185"/>
        <source>Switch to ship</source>
        <translation>转到舰船</translation>
    </message>
    <message id="techview-update-local">
        <location filename="../ClientGUI/ui/techview.ui" line="156"/>
        <source>Update Local Tech</source>
        <oldsource>Update</oldsource>
        <translation type="unfinished">更新局部科技</translation>
    </message>
    <message id="techview-equip-type">
        <location filename="../ClientGUI/ui/techview.ui" line="170"/>
        <source>EquipType:</source>
        <translation>装备类型：</translation>
    </message>
    <message id="techview-equip">
        <location filename="../ClientGUI/ui/techview.ui" line="177"/>
        <source>Equip:</source>
        <oldsource>Equipment:</oldsource>
        <translation>装备：</translation>
    </message>
    <message id="techview-ship-nationality">
        <location filename="../ClientGUI/ui/techview.ui" line="216"/>
        <source>Nationality:</source>
        <translation>国籍：</translation>
    </message>
    <message id="techview-ship-class">
        <location filename="../ClientGUI/ui/techview.ui" line="262"/>
        <source>ShipClass:</source>
        <translation>舰级：</translation>
    </message>
    <message id="techview-ship-type">
        <location filename="../ClientGUI/ui/techview.ui" line="229"/>
        <source>ShipType:</source>
        <translation>舰种：</translation>
    </message>
    <message id="techview-ship">
        <location filename="../ClientGUI/ui/techview.ui" line="275"/>
        <source>Ship:</source>
        <translation>舰船名：</translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/techview.ui" line="14"/>
        <source>Frame</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../ClientGUI/ui/techview.ui" line="57"/>
        <source>TextLabel</source>
        <translation></translation>
    </message>
</context>
</TS>
