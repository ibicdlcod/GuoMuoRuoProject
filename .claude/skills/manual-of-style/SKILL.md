...
6. Source file entries in set() lists (CLIENT_SOURCES, SERVER_SOURCES, PROTOCOL_SOURCES, etc.) must be in alphabetical order by full path. Pure string sort: .cpp sorts before .h (since 'c' < 'h'). When adding a new file, insert it at the correct alphabetical position.