...
Remove the `[NOTYETIMPLEMENTED]` suffix  
from the heading `### Maintenance of planes[NOTYETIMPLEMENTED]`
and replace the following paragraph with the detailed description below:
...
- After each battle, lost planes are automatically replenished;  
  the resource cost is deducted (negative balances are allowed).
- Cost is computed per equipment type, scaled from  
  the per‑100‑plane cost and rounded up.

- **Implementation references:**  
  – `PlaneReplenish::replenishAfterBattle()`  
  – Called from `Server::server_battle.cpp` line 1309.  

Place this new subsection after the updated Maintenance section  
and before the final paragraph about resources dropping below zero.
